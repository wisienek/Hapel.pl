...
const Role = () => {
    ...
    const [ roleCInfo, setroleCInfo ]           =   useLocalStorage( 'roleCInfo',       JSON.stringify({}) );
    ...
    const getPlayerInfo = () => {
        axios.get(`http://hapel-ic.pl/api/users/game`, {user}).then(res => {

            let data = res?.data?.rows? res.data.rows: [];
            if(data.length == 0)
                return addError({ type: "info", error: "Brak podłączonych kont z discordem w bazie!" });
        ...
        .catch(er => {
            ...
            return addError({ type: "warning", error: "Wystąpił błąd podczas pobierania kont z gry!" });
        });
    }

    const getServerRoles = () => {
        axios.get("http://hapel-ic.pl/api/role/server").then(data=> {
        ...
        .catch(er => {
            ...
            return addError({ type: "warning", error: "Wystąpił błąd podczas listy ról serwerowych!"});
        });
    }
    const getDiscordRoles = () => {
        axios.get("http://hapel-ic.pl/api/role/discord").then(data=> {
        ...
        .catch(er => {
            console.error(`Get server roles ${er}`);
            return addError({ type: "warning", error: "Wystąpił błąd podczas listy ról discordowych HIC!"});
        });
    }
    const getConnectedRoles = () => {
        axios.get("http://hapel-ic.pl/api/role/getConnected").then(res=> {
            let connectedRoles = res?.data?.role? { role: res.data.role, auto: res.data.autoRole, perms: res.data.perms }: { role: [], auto: [] };
        ...
        .catch(er => {
            ...
            return addError({ type: "warning", error: "Wystąpił błąd podczas listy ról Połączonych!" });
        });
    }
    ...
    if( !user || !user.hasPerms )
        return (<Container ><h1> Brak uprawnień! </h1></Container>);
        ...
        <>
            ...
            <Container fluid>
                ...
                <Route path="/role" exact>
                    <MainRole user={ user } parsedPlayer={ parsedPlayer } parsedCRole={ parsedCRole } />
                </Route> 

                <Route path="/role/wszystkie">
                    <AllRoles parsedCRole={ parsedCRole } parsedRole={ parsedRole } parsedRoleDC={ parsedRoleDC } user={ user } addError={ addError } />
                </Route>


                <Route path="/role/aplikacje">
                    <CreateRole parsedRole={ parsedRole } parsedRoleDC={ parsedRoleDC } user={ user } addError={ addError } />
                </Route>
