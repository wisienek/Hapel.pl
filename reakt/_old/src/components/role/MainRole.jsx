...
const MainRole = ({ user, parsedPlayer, parsedCRole }) => {
    ...
    const findRole = name => (parsedCRole.role || []).find(r=> r.name === name );
            ...
            <>
                {
                    role && role.canResign == 1?
                        <OverlayTrigger placement="auto" overlay={<Tooltip>Rezygnuj</Tooltip>} >
                ...
                <Popover.Content>
                    {
                        foundRole?
                            <>
                                <p><strong>Właściciel: </strong> { foundRole.creator || "Nieznany" }</p>
                                <p><strong>DisplayName: </strong> { foundRole.displayname || `[&7${name}&f]` }</p>
                                <p><strong>Parent: </strong> { foundRole.parent || "Brak" }</p>
                                <p><strong>Darmowa: </strong> { foundRole.isFree? "Tak": "Nie" }</p>
                                <p><strong>Aplikacje: </strong> { foundRole.canApply? "Tak": "Nie" }</p>
                                <p><strong>Rezygnacja: </strong> { foundRole.canResign? "Tak": "Nie" }</p>
                            </>
                        :
