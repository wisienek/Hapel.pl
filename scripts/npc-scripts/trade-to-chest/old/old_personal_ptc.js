...
function interact(e){
    ...
    if((mainh.getName()=="variedcommodities:blueprint" && mainh.getDisplayName().indexOf("NPC edit")>-1 && mainh.getLore()[0]=="Edytowanie") && (owner && owner==e.player.getUUID())){
    ...
    if((mainh.getName()=="variedcommodities:blueprint" && mainh.getDisplayName().indexOf("NPC edit")>-1) && (owner&&owner==e.player.getUUID()) && (tempData.get("edit")==true)){
        if(lore[0]=="Edytowanie"){
            if(tempData.get("edit")==true){
                ...
                tempData.put("edit",true);
                return e.player.message("[§cNPC§f] §7Włączono tryb edytowania! Wybierz teraz opcję (na blueprincie i PPM na NPC)");
            }
            ...
            return e.player.message("[§cNPC§f] §7Weź przedmiot 1 do łapy, walnij NPCta, weź 2 walnij, weź sprzedawane i walnij");
        }else if(lore[0]=="Usuń przedmiot"){
            tempData.put("mode","usuwanie");
            e.player.message("[§cNPC§f] §7Włączono tryb usuwania przedmiotów!");
            return e.player.message("[§cNPC§f] §7Napisz w książce jaki numer chcesz usunąć i walnij nią NPCta");
        }else if(lore[0]=="Edytuj wygląd"){
            tempData.put("mode","wygląd");
            e.player.message("[§cNPC§f] §7Włączono tryb edytowania wyglądu!");
            return e.player.message("[§cNPC§f] §7Zapisz w zeszycie: 1linijka- Nazwa/Skin, 2linijka- nazwa/LINK do skina");
        }else if(lore[0]=="Edytuj webhook"){
            tempData.put("mode","webhook");
            e.player.message("[§cNPC§f] §7Włączono tryb edytowania webhooka!");
            return e.player.message("[§cNPC§f] §7Zapisz w zeszycie: 1linijka: webhook, 2linijka: link do webhooka");
        }else{
            ...
            if(e.receiving === undefined){
                ...
                var hook = data.get("webhook");
                    if(getsold(e.sold, chest1,e,0)){
                        ...
                        e.setCanceled(true);
                        if(hook){
                            var x = "NPC: **"+e.npc.getName()+"** Nie ma towaru: **"+e.sold.getDisplayName()+"** ("+e.sold.getStackSize()+")";
                            x=ang(x);
                            HTTP.post(hook,{
                                "content": x,
                                "tts": false,
