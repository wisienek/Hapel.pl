function dialogOption(e){
    ...
    if(!wioski){return e.player.message("[§cPrzewoźnik§f] §7Coś poszło nie tak, brak zapisanych stopów!")}
    wioski = JSON.parse(wioski);
    opt+=1;
    if(name!="Więcej opcji"){
        var w1 = wioski[name];
        if(!w1){return e.player.message("[§cPrzewoźnik§f] §7Nie znaleziono stopa!")}

        if(maKase(e.player)==false){return e.player.message("[§cPrzewoźnik§f] §7Musisz mieć knuta na przejazd!")}

        e.player.message("[§cPrzewoźnik§f] §7Podróż zakończona, jesteś w: "+name+"!")
