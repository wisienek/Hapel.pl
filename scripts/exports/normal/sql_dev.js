...
function getBag(id)
{
    ...
    try
    {
        conn = DriverManager.getConnection(urlp, passes.sql_dev.login, passes.sql_dev.pwd);
...
function saveBag(uuid, nazwa, itemy, id)
{
    ...
    {
        conn = DriverManager.getConnection(urlp, passes.sql_dev.login, passes.sql_dev.pwd);
        var query = 'INSERT INTO main(id, uuid, nazwa, itemy, aktualizacja) VALUES('+id+', \''+uuid+'\', \''+nazwa+'\', \''+itemy+'\', '+aktualizacja+');';
        var st = conn.createStatement();
...
function updateBag(itemy, id){
    ...
    {
        conn = DriverManager.getConnection(urlp, passes.sql_dev.login, passes.sql_dev.pwd);
        var query = 'UPDATE main SET itemy = \''+itemy+'\', aktualizacja = '+aktualizacja+' WHERE id = '+id+';';
        var st = conn.createStatement();
        var rs = st.executeUpdate(query);
...
function saveBagCopy(id){
    ...
    try
    {
        ...
        if(!current || current.length==0){ return "Nie ma w bazie!"}
        current = current[0];

        conn = DriverManager.getConnection(urlp, passes.sql_dev.login, passes.sql_dev.pwd);
        var query = 'INSERT INTO kopie (id, uuid, nazwa, itemy, data) VALUES('+id+', \"'+current.uuid+'\", \"'+current.nazwa+'\", \''+escapeString(JSON.stringify(current.itemy).replace(/    /g,"").replace(/\n/g,""))+'\', '+current.aktualizacja+');';
        var st = conn.createStatement();
        var rs = st.executeUpdate(query);
