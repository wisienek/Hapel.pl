...
function attack(e){
    ...
    if(mainh.getName().toLowerCase().indexOf("air")>-1)
        return e.player.message("[§cNarzędzia§f] §7Musisz trzymać miotłę w 2 ręce!");
    ...
    if( opt == "naprawa" ){
        if( mainh.getName() == "variedcommodities:broken_arrow" ){
            ...
            if( time > Date.now() ){return e.player.message("[§cMiotły§f] §7Możesz naprawiać miotłę co 3 godziny!")}

            var miotly = JSON.parse(e.player.world.getStoreddata().get("miotly"));
            if(!miotly){ return e.player.message("[§cMiotły§f] §7Nie znaleziono spisu mioteł!!!") }
            ...
            var x = "**"+e.player.getName()+"** Naprawił miotłę narzędziami: **"+name+"** !";
            x=ang(x);
            ...
            return e.player.message("[§cNarzędzia§f] §7Musisz mieć złamaną miotłę w drugiej ręce!");
        }
    }else if(opt=="konserwacja"){
        ...
        for(var x=0; x < lore1.length; x++){
            if( lore[x].toLowerCase().indexOf("wytrzymałość: ")>-1 ){
                ...
                if(lore[x].split(": ")[1] == value) 
                    return e.player.message("[§cMiotła§f] §7Twoja miotła ma pełną wytrzymałość!");
