...
function init(e){
    ...
    e.item.setLore(["§7This item will create graphs with nodes","§7Each node by default is Player's position","§7To create a Node just §bRightClick","§7Once node is created you create edges for it.","§7If Node has no edges or you've finished adding them just §bLeftClick","§7When Node-Queue reaches §l0§7 it will save graph to worlddata!"]);
}
...
function interact(e){
    ...
    if(queue.length==0){
        ...
        graph.addNode(n);
        queue.push(n);
        e.player.message("[§cNode§f] §7Created new Node! Now Right-click to add an edge.");
    }else{
        ...
        if(n==undefined){
            ...
            queue.push(n);
            queue[0].addEdge(n);
            e.player.message("[§cNode§f] §7Added new node, edge of "+queue[0].value+", Queue length: §b"+queue.length);
