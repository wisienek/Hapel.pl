  ...
  run: async ({ client, message }) => {
    ...
    if (topNumber) {
      if (topNumber <= 0 || topNumber > 20)
        return message.reply(`Topka ograniczona: (0; 20>`);
    ...
    if (info) {
      const elek: Oczekujace = await client.db.oczekujace.findFirst({
        where: { discord: message.user.id, id: info },
      });
      if (!elek) return message.reply(`Nie znaleziono elka o id: ${info}`);

      const eliksir: Eliksiry = await client.db.eliksiry.findFirst({
        where: { nazwa: elek.eliksir },
      });
      if (!eliksir)
        return message.reply(
          `Nie znaleziono odpowiadającego eliksiru: ${elek.eliksir}`
        );
      ...
      const embed = new MessageEmbed()
        ...
        .addField(
          'Weryfikowane',
          elek.weryfikowane == 1 ? 'Tak' : 'Oczekuje',
          true
        )
        ...
        .addField(
          'Data odebrania',
          elek.data_odebrania
            ? new Date(elek.data_odebrania).toDateString()
            : 'Brak danych',
          true
        )
        .addField(
          'Kociołek',
          elek.kociolek ? elek.kociolek : 'Cynowy kociołek rozmiar 2',
          true
        )
        //.addBlankField()
        ...
        .addField(
          'Czas trwania',
          isNaN(Number(eliksir.czas)) ? eliksir.czas : `${eliksir.czas} minut`,
          true
        )
      ...
      for (const elek of elki) {
        const embed = new MessageEmbed()
          ...
          .addField(
            'Data odebrania',
            elek.data_odebrania
              ? new Date(Number(elek.data_odebrania)).toDateString()
              : 'Brak danych',
            true
          )
          .addField(
