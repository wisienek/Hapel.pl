...
const UserInfo = () => {
  ...
  const [searched, setSearched] = useState(
    Location.pathname.match(/\d{18}/g) || []
  );
  ...
  const [parsedPlayer, setParsedPlayer] = useState(
    searched ? [] : player.length > 2 ? JSON.parse(player) : []
  );
  ...
  const wyloguj = async () => {
    await axios
      .get('http://hapel-ic.pl/api/dc/logout')
      .then((res) => {
        let x = res?.data?.type
          ? res.data
          : { type: 'info', error: 'brak wiadomości!' };
      ...
      .catch((er) =>
        er.type && er.error
          ...
          : addError({
              type: 'danger',
              error: `Wystąpił nieoczekiwany błąd: ${er}`,
            })
      );
  };
  ...
  const getPlayerInfo = async (id) => {
    ...
    await axios
      .get(
        `http://hapel-ic.pl/api/users/game${
          id.length > 0 ? `?id=${id[0]}` : ''
        }`,
        { user }
      )
      .then((res) => {
        ...
        if (data.length == 0)
          return addError({
            type: 'info',
            erorr: 'Brak podłączonych kont z discordem w bazie!',
          });
  ...
  const getHicInfo = async (id) => {
    await axios
      .get(
        `http://hapel-ic.pl/api/users/hic${
          id.length > 0 ? `?id=${id[0]}` : ''
        }`,
        { user }
      )
      ...
      .catch((er) => {
        ...
        return addError({
          type: 'warning',
          error: 'Wystąpił błąd podczas pobierania info o koncie discord!',
        });
      });
  };
  const getSUser = async (id) => {
    ...
    await axios
      .get(
        `http://hapel-ic.pl/api/users/web${
          id.length > 0 ? `?id=${id[0]}` : ''
        }`,
        { user }
      )
      .then((res) => {
        ...
        if (!data)
          return addError({
            type: 'danger',
            error: 'Brak wyników po fetchowaniu usera!',
          });

        axios
          .get(data.avatar.replace('webp', 'gif'))
          .then((a) => {
  ...
  const toggleEdit = (nick) => {
    !displayEdit[nick] || displayEdit[nick] === false
      ? setDisplayEdit({ ...displayEdit, [nick]: true })
      : setDisplayEdit({ ...displayEdit, [nick]: false });
  ...
  const editInfo = (e, ind) => {
    ...
    if (data.plec && 'mf'.indexOf(data.plec.toLowerCase()) == -1)
      return addError({
        type: 'warning',
        error: 'Podano złą płeć: m - mężczyzna, f - kobieta',
      });
    if (
      data.plec == parsedPlayer[ind].plec &&
      data.wiek == parsedPlayer[ind].wiek &&
      data.opis == parsedPlayer[ind].opis
    )
    ...
    axios
      .get(
        `http://hapel-ic.pl/api/users/edit?${Object.keys(data)
          .map((l) => (l += '=' + data[l]))
          .join('&')}`
      )
      .then((res) => {
  ...
  const archive = (nick) => {
    if (!nick)
      return addError({
        type: 'warning',
        error: 'Nie podano nicku do archiwizacji!',
      });

    setLoading(true);
    axios
      .patch(`http://hapel-ic.pl/api/users/archive?nick=${nick}`, { user })
      .then((res) => {
        addError(res.data);
        return getPlayerInfo(searched);
      })
      .catch((er) => {
        if (er.error && er.type) addError(er);
  ...
  const toggleVisible = (nick) => {
    if (!nick)
      return addError({
        type: 'warning',
        error: 'Nie podano nicku do zmiany widoczności!',
      });
    setLoading(true);

    axios
      .patch(`http://hapel-ic.pl/api/users/visible?nick=${nick}`, { user })
      .then((res) => {
        addError(res.data);
        return getPlayerInfo(searched);
      })
  ...
  return (
    <>
      <Main
        searched={searched}
        sUser={searched.length > 0 ? sUser : user}
        user={user}
        loading={loading}
      ...
      <Characters
        ...
        user={user}
        archive={archive}
        players={searched.length == 0 ? parsedPlayer : Splayer}
        searched={searched}
