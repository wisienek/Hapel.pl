...
const Glowna = () => {
  ...
  const getDataDaily = () => {
    ...
    axios
      .get(`http://hapel-ic.pl/api/help/daily`, { user })
      .then((res) => {
  ...
  const getDailyInfo = () => {
    ...
    let med =
      help.length / 2 == parseInt(help.length / 2)
        ? help[help.length / 2].ilosc
        : (help[Math.floor(help.length / 2)].ilosc +
            help[Math.ceil(help.length / 2)].ilosc) /
          2;
  ...
  const getServerInfo = () => {
    axios
      .get('https://api.mcsrvstat.us/2/51.83.193.68:25923')
      .then((res) => {
    ...
    try {
      discords.forEach(async (dc) => {
        const res = await axios.get(
          `https://discord.com/api/guilds/${dc}/widget.json`
        );
        const res1 = await axios.get(
          `https://discordapp.com/api/v6/invite/${
            res.data.instant_invite.split('invite/')[1]
          }?with_counts=true`
        );
