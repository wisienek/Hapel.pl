...
const Plan = ({ user, addError }) => {
  ...
  const handleDelete = (_def) => {
    ...
    axios
      .delete(`http://hapel-ic.pl/api/hogwart/plan`, {
        data: { def: _def, user },
        headers: { Authorization: '***' },
      })
      .then((res) => {
        if (res.data.message) addError(res.data);

        setDef({});
        setCurrentPlan(currentPlan.filter((p) => p.id != _def.id));
      })
      .catch((er) => {
        if (er?.response?.type && er?.response?.error) addError(er.response);
        console.log(er);
      });
  };
  ...
  const changeNotify = async () => {
    try {
      if (!user) throw 'Brak użytkownika!';

      const res = await axios.patch(
        `http://hapel-ic.pl/api/hogwart/planfollow?year=${year}`,
        user
      );
      if (res.data.type) addError(res.data);
      setSubscribed({
        subscribed,
        ['kl' + year]: subscribed['kl' + year] == 1 ? 0 : 1,
      });
    } catch (er) {
      console.error(er);
    }
  ...
  const getTeachers = () => {
    axios
      .get(`http://hapel-ic.pl/api/hogwart/teachers`)
      .then((res) => {
  ...
  const getSubscribed = async () => {
    try {
      if (!user) return console.log('Brak użytkownika!');

      const res = await axios.get(
        `http://hapel-ic.pl/api/hogwart/planfollow`,
        user
      );
  ...
  const addLesson = (e) => {
    ...
    let { _class, place, day, start, end, name, prof, edit, temp, when } =
      e.target;
    ...
    if ((edit == true && !def.id) || (temp == true && !def.id))
      return addError({
        type: 'danger',
        message: 'Brak ID edytowanej lekcji!',
      });

    if (!temp && [1, 2, 3, 4].indexOf(_class) == -1)
      return addError({
        type: 'warning',
        error: `Błędny argument w wybranej klasie: ${_class}`,
      });
    if (!temp && (day < 0 || day > 6))
      return addError({
        type: 'warning',
        error: `Błędny argument w wybranym dniu: ${day}`,
      });
    if (
      !temp &&
      (start.match(/\d{2}\:\d{2}/) == null || end.match(/\d{2}\:\d{2}/) == null)
    )
      return addError({
        type: 'warning',
        error: `Błędny argument w wybranych godzinach: ${start}, ${end}`,
      });
    if (
      !temp &&
      (!name ||
        name.length < 3 ||
        name.replace(/\W/g, '').length < 3 ||
        place.length < 3 ||
        place.replace(/\W/g, '').length < 3)
    )
      return addError({
        type: 'warning',
        error: `Błędny argument w nazwie lub miejscu: ${name}, ${place}`,
      });
    if (!temp && (typeof prof != 'object' || !prof.displayName || !prof.nick))
      return addError({
        type: 'warning',
        error: `Niepoprawny profesor: ${JSON.stringify(prof)}`,
      });
    ...
    (edit === true
      ? axios.patch(`http://hapel-ic.pl/api/hogwart/plan`, {
          lekcja: data,
          user,
        })
      : temp === true
      ? axios.put(`http://hapel-ic.pl/api/hogwart/plan`, { lekcja: data, user })
      : axios.post(`http://hapel-ic.pl/api/hogwart/plan`, {
          lekcja: data,
          user,
        })
    )
  ...
  const getPlan = () => {
    axios
      .get(`http://hapel-ic.pl/api/hogwart/plan`)
      .then((res) => {
        if (res.data.plan) {
          let plan = res.data.plan.map((l) => {
            return {
              ...l,
              prof: JSON.parse(l.prof),
              Sprof: l.Sprof ? JSON.parse(l.Sprof) : null,
            };
          });

          setCurrentPlan(plan);
          setSelectedPlan(
            plan.filter((p) => p.day == days.indexOf(day) && p.class == year)
          );
      ...
      <div className="grid80">
        <Card>
          ...
          <Card.Header className="planCardHeader">
            <ButtonGroup>
              <Button
                variant={`${year == 1 ? '' : 'outline-'}primary`}
                name="year"
                value={1}
              ...
              <Button
                variant={`${year == 2 ? '' : 'outline-'}success`}
                name="year"
                value={2}
                onClick={(e) => changeClass(e)}
              >
              ...
              <Button
                variant={`${year == 4 ? '' : 'outline-'}secondary`}
                name="year"
                value={4}
                onClick={(e) => changeClass(e)}
              >
