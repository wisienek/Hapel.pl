...
const CreateRole = ({ user, parsedRole, parsedRoleDC, addError }) => {
    ...
    const addRole = (data) => {
        ...
        axios.post(`http://hapel-ic.pl/api/role/nowy`, { user, params: data })
        .then(res=> {
            if( res.data )
                addError( res.data );
    ...
    const multiDcRole = () => {
        const role = ( (user && user.hasPerms)? parsedRoleDC: parsedRoleDC.filter(r=> r.permissions < 238542409) ) || [];
    ...
    return (
        <div className="content">
            <h1>Stwórz rolę</h1>
            <SRole user={user} mappedRoles={mappedRoles()} parsedRoleDC={parsedRoleDC} parsedRole={parsedRole} addRole={addRole} multiDcRole={multiDcRole} create={create} setCreate={setCreate} add={add} setAdd={setAdd} index={1} />
