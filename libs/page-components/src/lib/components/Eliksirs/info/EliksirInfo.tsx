...
export const EliksirInfo = ({
  ...
  const akceptuj = async (e: any) => {
    e.preventDefault();
    if (!user) return blad({ type: 'danger', error: 'Nie jesteś zalgoowany!' });
    if (!user.hasPerms)
    ...
    const data = {
      ...
      user: user.username,
      permissions: user.guilds.filter((g) => g.id === '133227600746250240')[0]
        .permissions,
    };

    await axios
      .post(`http://hapel-ic.pl/api/elki/verify`, data)
      .then((res) => {
        if (res.data.ok) {
          ...
          blad(
            res.data.error || { type: 'danger', error: 'Nieoczekiwany błąd' }
          );
        }
      })
      .catch((er) => {
          ...
          <Card.Body>
            <Card.Title>
              ...
              <Col style={{ flexGrow: '2' }}>
                <Badge
                  ...
                  variant={eliksir.weryfikowane == 1 ? 'success' : 'danger'}
                >
                  Weryfikowany
                </Badge>
                <Badge
                  style={{ marginRight: '5px' }}
                  variant={eliksir.odebrane == 1 ? 'success' : 'danger'}
                  className={eliksir.odebrane == 1 ? 'hoverContainer' : ''}
                >
                  Odebrane
                  {eliksir.odebrane == 1 ? (
                    <span className="hoverText">
                      {new Date(
                        Number(eliksir.data_odebrania)
                      ).toDateString() || 'Brak daty odebrania'}
                    </span>
                  ) : (
                    ''
                  )}
                </Badge>
                <Badge style={{ marginRight: '5px' }} variant="warning">
                  Cena <Badge variant="light">{eliksir.cena}</Badge>
                </Badge>
              </Col>
            ...
            <div style={{ display: 'flex' }} className="sm-col">
              <Col>
                {eliksir.discord ? (
                  <Link
                    to={`/user/${eliksir.discord}`}
                    style={{ color: '#c5c9c5' }}
                  >
                    <h5 className="hoverContainer">
                      Nick: {eliksir.gracz || 'Nieznany'}{' '}
                      <span className="hoverText">{eliksir.uuid || ''}</span>{' '}
                    </h5>
                  </Link>
                ) : (
                  <h5 className="hoverContainer">
                ...
                <p>
                  Discord: {eliksir.discord ? `<@${eliksir.discord}>` : 'Brak'}
                </p>
                <p>Kociołek: {eliksir.kociołek || 'Cynowy rozmiar 2'}</p>
              </Col>
              <Col>
                <ol className="elki_przepis">
                  {JSON.parse(eliksir.przepis.replace(/§./g, '')).map(
                    (krok, index) => (
                      <li key={index}>{krok}</li>
                    )
                  )}
                </ol>
              </Col>
            ...
            {user && user.hasPerms == true && eliksir.weryfikowane != 1 ? (
              <>
                <div
                  id={`deny-${eliksir.id}`}
                  className={stan[ind] == 1 ? '' : 'toggle-visible'}
                >
                ...
                <div
                  id={`akcept-${eliksir.id}`}
                  className={stan[ind] == 2 ? '' : 'toggle-visible'}
                >
                    ...
                    <Form.Row>
                      <Col>
                        <Form.Group>
                          ...
                          <Form.Text className="text-muted">
                            Gracz otrzyma z reguły 4 fiolki jeżeli nie zmienisz
                            wartości.
                          </Form.Text>
                        </Form.Group>
                      </Col>
                      <Col>
                        <Form.Group>
                          ...
                          />
                          <Form.Text className="text-muted">
                            14 dni to podstawowa wartość o ile nie ma customowej
                            daty.
                          </Form.Text>
                        </Form.Group>
                      </Col>
                ...
                <Button
                  variant="danger"
                  onClick={() => updateStan(ind, stan[ind] === 1 ? 0 : 1)}
                  style={{ float: 'left' }}
                >
                  Odrzuć
                </Button>
                <Button
                  variant="success"
                  onClick={() => updateStan(ind, stan[ind] == 2 ? 0 : 2)}
                  style={{ float: 'right' }}
                >
                  Zaakceptuj
                </Button>
