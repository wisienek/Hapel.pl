...
const Cennik = ({ blad }) => {
  ...
  const getSkladniki = async () => {
    if (!skladniki || skladniki.length == 0) {
      axios
        .get(`http://hapel-ic.pl/api/elki/skladniki`)
        .then((res) => {
          const arr = res?.data?.rows ? res.data.rows : '';
          if (arr == '')
            return blad({
              type: 'danger',
              error: 'Nie udało się fetchować informacji o składnikach!',
            });
          let final = {};
  ...
  const dodajSkladnik = () => {
    ...
    if (parsedSkladniki[nazwa]) {
      ...
      if (!ile || ile <= 0 || ile > 4000)
        return blad({
          type: 'warning',
          error: 'Zła ilość. Musi się znajdować w przedziale (0; 4000)',
        });

      let wart = Math.ceil(
        ile * (parsedSkladniki[nazwa].cena / parsedSkladniki[nazwa].ilosc)
      );
      let final = {
        id: lista.length + 1,
        nazwa,
  ...
  return (
    <>
      {wx ? (
        <nav>
          <Link to="/elki/utility">
            {' '}
            <Button
              variant={
                Location.pathname == '/elki/utility' ? 'primary' : 'secondary'
              }
            ></Button>
          </Link>
          <Link to="/elki/utility/tabelka">
            <Button
              variant={
                Location.pathname == '/elki/utility/tabelka'
                  ? 'primary'
                  : 'secondary'
              }
            ></Button>
      ...
      <div className="content">
          ...
          <Form id="skladnikSelect">
            ...
            <Form.Row>
              <Col>
                <OverlayTrigger
                  placement="auto"
                  overlay={<Tooltip>Typ składnika</Tooltip>}
                >
                  <input
                    className="form-control"
                    id="typ"
                    type="text"
                    placeholder="Typ składnika"
                    disabled
                  />
                </OverlayTrigger>
              </Col>
              <Col>
                <OverlayTrigger
                  placement="auto"
                  overlay={<Tooltip>Jednostka</Tooltip>}
                >
                  <input
                    className="form-control"
                    id="jednostka"
              ...
              <Col>
                <OverlayTrigger
                  placement="auto"
                  overlay={<Tooltip>ile jednostek</Tooltip>}
                >
                  <input
                    className="form-control"
                    id="ilosc"
              ...
              <Col>
                <OverlayTrigger
                  placement="auto"
                  overlay={<Tooltip>cena za X jednostek</Tooltip>}
                >
                  <input
                    className="form-control"
                    id="cena"
            ...
            <Button variant="primary" onClick={() => dodajSkladnik()}>
              Dodaj składnik
            </Button>
          </Form>
        </Card>
        ...
        {lista && lista.length > 0 ? (
          <Card>
            <Card.Header>
              <h2>Aktualna cena: {cena + ' knutów'}</h2>
            </Card.Header>
              ...
              {lista.map((l, ind) => (
                <ListGroup.Item key={ind}>
                  ...
                  <button
                    type="button"
                    className="close"
                    style={{ float: 'right', color: '#b2becd' }}
                    onClick={() => usunSkladnik(l.id)}
                  >
                    X
                  </button>
                </ListGroup.Item>
