...
function interact(e) {
    ...
    if (naprawa[e.player.getName()]) {
      ...
      if (v < 1) {
        return e.player.message(
          '[§cMiotły§f] §7Jeszcze nie minęła godzina naprawy! (' +
            (1 - v).toFixed(2) +
            ')'
        );
      } else {
        var tempm = e.player.world.createItemFromNbt(
          API.stringToNbt(naprawa[e.player.getName()]['item'])
        );
        var miotly = JSON.parse(e.player.world.getStoreddata().get('miotly'));
        if (!miotly) {
          return e.player.message(
            '[§cMiotły§f] §7Nie znaleziono spisu mioteł!!! Pisz szybko do Woolfa'
          );
        }
        ...
        return e.player.message(
          '[§cMiotły§f] §7Dostałeś miotłę spowrotem z naprawy!'
        );
      }
    }
  } else {
  ...
  if (mainh.getName() == 'variedcommodities:broken_arrow') {
    if (wallet(e.player.getName() >= 90)) {
      ...
      return e.player.message(
        '[§cMiotły§f] §7Przyjęto miotłę! Przyjdź za godzinę aby odebrać naprawioną.'
      );
    } else {
