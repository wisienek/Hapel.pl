...
function interact(e){
if(akcje==true){return e.player.message("[§cMiotły§f] §7Musisz przesłać zaległe akcje aby odblokować narzędzie")}
    ...
    if(players && players.indexOf(e.player.getName())>-1){
        ...
        if(mainh.getName()=="variedcommodities:broken_arrow"){
            ...
            if(time){
                ...
                if(v<2){return e.player.message("[§cMiotły§f] §7Możesz naprawiać miotłę co 3 godziny! ("+(3-v).toFixed(2)+")")}
            }
            ...
            var x = "**"+e.player.getName()+"** Naprawił miotłę: **"+name+"** !";
            x=ang(x);
