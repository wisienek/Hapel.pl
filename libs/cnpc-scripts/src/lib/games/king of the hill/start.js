...
function interact(e) {
  ...
  if (state == 'start') {
    return e.player.message('[§cGame§f] §7Gra jest już wystartowana!');
  }
  ...
  if (players.length < 3) {
    return e.player.message('[§cGame§f] §7Musi być przynajmniej 3 graczy!');
  }
  state = 'start';
...
function timer(e) {
  if (e.id == 404) {
    ...
    var players = wdata.get('hillplayers');
    if (!king) {
      broadcast('Koniec gry! Nikt nie wygrał rundy.');
    } else {
      broadcast('Wygrał §e' + king + '§7!');
    }
    var p = API.getIWorlds()[0].getPlayer(king);
    if (p) {
      var item = API.getIWorlds()[0].createItem(
      ...
      item.setLore([
        '§7Wygrana w grze King of the hill',
        '§7Jesteś jednym z najlepszych obrońców!',
      ]);
      var item1 = API.getIWorlds()[0].createItem(
      ...
      item1.setLore([
        '§7Pieniążek zwycięzcy',
        '§7Możesz go wymienić na różne ulepszenia!',
      ]);
