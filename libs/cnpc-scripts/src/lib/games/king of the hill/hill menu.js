...
function gui3(player) {
  ...
  var zasady =
    '1. W grze może brać udział nieograniczona ilość graczy.||2. Każda runda trwa 5 min.||3. Grę wygrywa osoba, która jako ostatnia posiada tytuł króla.||4. Aby zdobyć Koronę należy dojść do tronu i przejąć ją z niego.||5. Król może stracić koronę przez wyjście z tronu.||6. Nagrodą za ukończenie gry jako król jest korona.';
  zasady = zasady.split('||');
...
function customGuiButton(e) {
  switch (e.buttonId) {
    case 1: {
      ...
      if (state == 'start') {
        return e.player.message('[§cGame§f] §7Gra jest już aktywna!');
      }
      if (!players) {
        players = [];
      } else {
    ...
    case 11: {
      ...
      var state = wdata.get('hill');
      if (state == 'start') {
        return e.player.message('[§cGame§f] §7Gra jest aktywna!');
      }
      if (!players) {
        players = [];
      } else {
