...
function init(e) {
  ...
  wdata.put(
    'crown',
    JSON.stringify([e.block.getX(), e.block.getY(), e.block.getZ()])
  );
}

function interact(e) {
  var wdata = API.getIWorlds()[0].getStoreddata();
  if (!wdata.get('hill') || wdata.get('hill') == 'stop') {
    return e.player.message('[§cGame§f] §7Gra jest zatrzymana!');
  }
  ...
  if (players.indexOf(e.player.getName()) == -1) {
    return e.player.message('[§cGame§f] §7Nie jesteś w grze!');
  }
