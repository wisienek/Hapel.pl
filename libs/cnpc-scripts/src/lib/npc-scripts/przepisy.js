    ...
    for (var i = 0; i < przepisy.length; i++) {
      lista.push(
        przepisy[i].id +
          ...
          przepisy[i].cena +
          ' §e' +
          stars[0].repeat(przepisy[i].ocena) +
          stars[1].repeat(5 - przepisy[i].ocena)
      );
    }
  } else {
    lista = search;
...
function customGuiButton(e) {
  switch (e.buttonId) {
    ...
    case 11: {
      ...
      return e.player.message(
        '[§cPrzepisy§f] §7Zmieniono tryb na §e' + x + '!'
      );
    }
    case 16: {
      ...
      try {
        ...
        var lista1 = [];
        for (var i = 0; i < len; i++)
          if (lista[i].toLowerCase().indexOf(text) > -1) lista1.push(lista[i]);
    ...
    case 201: {
      ...
      if (cena < 0 || cena > 99999 || isNaN(cena)) {
        return e.player.message(
          '[§cPrzepisy§f] §7Podano niepoprawną cenę. ' + cena
        );
      }
      ...
      if (!tempdata.has('przepis')) {
        ...
        if (!przepis.cena || przepis.cena != cena) {
          przepis.cena = cena;
        }
        if (!przepis.autor) {
          przepis.autor = e.player.getName();
        }
    ...
    case 202: {
      ...
      if (ocena < 1 || ocena > 5 || isNaN(ocena)) {
        return e.player.message(
          '[§cPrzepisy§f] §7Podano niepoprawną ocenę: §e' + ocena
        );
      }
    ...
    case 303: {
      ...
      if (npc) {
        ...
        if (wyplaty[e.player.getName()]) {
          ...
          if (x != true) {
            return e.player.message('[§cDebugger§f] §7Error: ' + x);
          }
          ...
          e.player.message(
            '[§cPrzepisy§f] §7Odebrałeś wypłatę za przepisy: §e' + cena
          );
          return menu(e);
        } else {
          return e.player.message(
      ...
      if (inf == 'Kupowanie' || inf == 'Admin' || inf == 'Usuń') {
        ...
        if (inf == 'Usuń') {
          if (
            ...
            return e.player.message(
              '[§cPrzepisy§f] §7Usunięto przepis o id: §e' + id
            );
          } else {
            return e.player.message(
        ...
        if (inf == 'Admin' || wallet(e.player.getName()) >= eliksir.cena) {
          var x =
            inf == 'Admin'
              ? true
              : requestPayment(e.player.getName(), eliksir.cena);
          ...
          if (inf != 'Admin') {
            var data = npc.getStoreddata();
            var wyplaty = JSON.parse(data.get('wyplaty')) || {};
            wyplaty[eliksir.autor]
              ? (wyplaty[eliksir.autor] += parseInt(0.9 * eliksir.cena))
              : (wyplaty[eliksir.autor] = parseInt(0.9 * eliksir.cena));
            data.put('wyplaty', JSON.stringify(wyplaty));
          ...
          var item = e.player.world.createItemFromNbt(
            e.API.stringToNbt(eliksir.item)
          );
          ...
          return e.player.message(
            '[§cPrzepisy§f] §7Masz za mało kasy: §e' + eliksir.cena
          );
        }
      } else {
...
function customGuiSlot(e) {
  if (e.slotId == 0) {
    var item = e.stack;
    if (item.getName().indexOf('minecraft:book') == -1) {
