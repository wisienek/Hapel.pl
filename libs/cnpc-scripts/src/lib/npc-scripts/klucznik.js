function interact(e) {
  ...
  if (list[e.player.getName()]) {
    return e.player.message(
      '[§cKlucznik§f] §7Już dostałeś jedną skrzynkę: ' +
        list[e.player.getName()]
    );
  }
  ...
  var found;
  var blocks = [
    e.npc.world.getBlock(pos.getX(), pos.getY() - 2, pos.getZ()),
  ...
  try {
          ...
          for (var i = 0; i < items.length; i++) {
            if (items[i].getName().indexOf('air') == -1) {
              if (klucze.indexOf(items[i].getDisplayName()) == -1) {
                ...
                return e.player.message(
                  '[§cKlucznik§f] §7Zajęto skrzynkę: ' + found.getDisplayName()
                );
              }
            }
          }
    ...
    if (!found) {
      return e.npc.say(
        '[§cKlucznik§f] §7Wszystkie magazyny zostały już zajęte!'
      );
    }
  } catch (er) {
    print(er);
