    ...
    if(mainh.getName()=="minecraft:stick"){
        ...
        if(data.get("teach")==1){
            ...
            if(pet){
                ...
                nbt.setString("food_chest", block.getX()+" "+block.getY()+" "+block.getZ());
...
function interact(e){
    ...
    if(mainh.getName()=="minecraft:stick"){
        ...
        if(hunger+ammount > 100){ ammount = 100 } else { ammount += hunger }
...
function timer(e){
    switch(e.id){
        case 1:{
            ...
            if(hunger<=20){
                e.npc.say("*growls*");
                if(!e.npc.getRole().isFollowing() && nbt.getString("food_chest")){//not following and chest is set
