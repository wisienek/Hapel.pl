...
function interact(e) {
  ...
  if (
    (mainh.getName() == 'variedcommodities:blueprint' &&
      ...
      ((owner && owner.indexOf(e.player.getName()) > -1) ||
        owner.indexOf(e.player.getUUID()) > -1)) ||
    Object.keys(admins).indexOf(e.player.getUUID()) > -1
  ) {
    e.setCanceled(true);
...
function gui2(player) {
  ...
  gui
    .addLabel(10, '§9Nazwa', 65, 80, 25, 15)
    .setHoverText([
      '§4Uwaga!',
      '§7Aby dodać kolor do nazwy',
      '§7Poprzedź tekst znakiem §l&§r§7 i kodem koloru (patrz niżej)',
      '§e&eZłoty król',
    ]);
  ...
  gui
    .addLabel(12, '§9Skin', 70, 100, 15, 15)
    .setHoverText([
      '§4Uwaga!',
      '§7Skin musi być zapisany na platformie imgur',
      '§7Wpisz tylko końcówkę linku zaznaczoną zielonym kolorem poniżej',
      '§chttps://i.imgur.com/§aDEiZteW.png',
    ]);
...
function gui4(player) {
  ...
  gui
    .addTextField(11, 50, 65, 140, 20)
    .setHoverText(['§7Wklej tutaj cały webhook z discorda']);
  gui
    .addButton(201, '§a✍', 200, 65, 20, 20)
    .setHoverText(['§7Zapisz webhook']);
...
function gui5(player, usun, search) {
  ...
  if (usun) {
    ...
    for (var i = 0; i < keys.length; i++) {
      if (
        search &&
        admins[keys[i]].toLowerCase().indexOf(search.toLowerCase()) > -1
      ) {
    ...
    for (var i = 0; i < players.length; i++) {
      if (
        search &&
        players[i].getName().toLowerCase().indexOf(search.toLowerCase()) > -1
      ) {
  ...
  gui
    .addLabel(19, '§4§l[?]', 10, 40, 15, 15)
    .setHoverText([
      '§7Kliknij §edwukrotnie§7 na osobę z listy',
      '§7Aby dodać/usunąć ją jako współwłaściciela',
      '§7Współwłaściciel ma §eTakie same §7prawa co właściciel',
    ]);
...
function customGuiScroll(e) {
  switch (e.scrollId) {
    case 11: {
      ...
      if (!sowner) {
        return e.player.message('[§cNPC§f] §7Nie znaleziono gracza!');
      }
      ...
      e.player.message(
        '[§cNPC§f] §7Dodano gracza §b' + name + '§7 jako współwłaściciela!'
      );
    ...
    case 17: {
      ...
      if (keys.indexOf(id) == -1) {
        return e.player.message('[§cNPC§f] §7Nie ma takiego admina!');
      }
      ...
      e.player.message(
        '[§cNPC§f] §7Usunięto gracza §b' + name + '§7 ze współwłaścicieli!'
      );
...
function customGuiButton(e) {
  switch (e.buttonId) {
    case 1: {
      ...
      item.setLore([
        '§7Tajemniczy kryształ trzyma w sobie',
        '§7Duszę androida sprzedawcy!',
        npc.getName(),
        '§8Kliknij Prawym przyciskiem myszy aby ją uwolnić',
      ]);
    ...
    case 8: {
      ...
      if (!rozmiar || isNaN(rozmiar) || rozmiar <= 0 || rozmiar > 7) {
        return e.player.message('[§cNPC§f] §7Rozmiar musi być z zakresu <1;7>');
      }
      if (!npc) {
        e.player.message('nie ma npc');
      }
      if (rozmiar == 7) {
        rozmiar = 1;
      } else {
    ...
    case 13: {
      ...
      if (!text) {
        return e.player.message('[§cNPC§f] §7Nie ma tekstu w polu!');
      }
    ...
    case 200: {
      ...
      if (skin && skin != display.getSkinUrl().split('/').pop()) {
        ...
        e.player.message(
          '[§cNPC§f] §7Zmieniono skin na §bhttps://i.imgur.com/' + skin
        );
      }
      return gui2(e.player);
    }
    case 201: {
      ...
      if (
        text.indexOf('discord') == -1 ||
        text.indexOf('api') == -1 ||
        text.indexOf('webhook') == -1
      ) {
...
function customGuiSlot(e) {
  ...
  var id;
  if (
    (e.slotId >= 0 && e.slotId <= 5) ||
    (e.slotId >= 18 && e.slotId <= 23) ||
    (e.slotId >= 36 && e.slotId <= 41)
  ) {
    slot1 = e.stack;
  } else if (
    (e.slotId >= 6 && e.slotId <= 11) ||
    (e.slotId >= 24 && e.slotId <= 29) ||
    (e.slotId >= 42 && e.slotId <= 47)
  ) {
    slot2 = e.stack;
  } else if (
    (e.slotId >= 12 && e.slotId <= 17) ||
    (e.slotId >= 30 && e.slotId <= 35) ||
    (e.slotId >= 48 && e.slotId <= 53)
  ) {
    slot3 = e.stack;
  }

  if (e.slotId >= 0 && e.slotId <= 5) {
    id = e.slotId;
  } else if (e.slotId >= 6 && e.slotId <= 11) {
    id = e.slotId - 6;
  } else if (e.slotId >= 12 && e.slotId <= 17) {
    id = e.slotId - 12;
  } else if (e.slotId >= 18 && e.slotId <= 23) {
    id = e.slotId - 12;
  } else if (e.slotId >= 24 && e.slotId <= 29) {
    id = e.slotId - 18;
  } else if (e.slotId >= 30 && e.slotId <= 35) {
    id = e.slotId - 24;
  } else if (e.slotId >= 36 && e.slotId <= 41) {
    id = e.slotId - 24;
  } else if (e.slotId >= 42 && e.slotId <= 47) {
    id = e.slotId - 30;
  } else if (e.slotId >= 48 && e.slotId <= 53) {
    id = e.slotId - 36;
  }

  if (slot1) {
    nrole.set(id, slot1, nrole.getCurrency2(id), nrole.getSold(id));
  } else if (slot2) {
    nrole.set(id, nrole.getCurrency1(id), slot2, nrole.getSold(id));
  } else if (slot3) {
    nrole.set(id, nrole.getCurrency1(id), nrole.getCurrency2(id), slot3);
  }
  return;
}
