 ...
 * trade_to_chest.js
 *
 * CNCP script for Player's trader
 *
...
function interact(e) {
  ...
  if (
    (mainh.getName() == 'variedcommodities:blueprint' &&
      ...
      owner &&
      owner.indexOf(e.player.getName()) > -1) ||
  ...
  HTTP = {
    ...
    post: function (e, t) {
      ...
      try {
        ...
        p.writeBytes(new String(JSON.stringify(t))),
          p.flush(),
          p.close(),
          a.close();
      } catch (e) {}
...
function gui2(player) {
  ...
  gui
    .addLabel(10, '§9Name', 65, 80, 25, 15)
    .setHoverText([
      '§4Warning!',
      '§7To add coloured name',
      '§7add prefix with sign §l&§r§7 and color code like presented below',
      '§e&eShopkeeper',
    ]);
  ...
  gui
    ...
    .setHoverText([
      '§4Warning!',
      '§7Skin Has to be saved on imgur',
      '§7Put in just the last part of the link like shown below',
...
function gui4(player) {
  ...
  gui
    ...
    .setHoverText(['§7Here you can paste in your &lDiscord&r&7 webhook']);
...
function gui5(player, usun, search) {
  ...
  gui
    ...
    .setHoverText([
      '§7Click §eTwice§7 on a person from list',
      '§7To add/remove them from admin list',
      '§7Admins have the same rights as owner!',
    ]);
...
function customGuiScroll(e) {
  switch (e.scrollId) {
    case 11: {
      ...
      if (!sowner) {
        return e.player.message('[§cNPC§f] §7Player not found!');
      }
    ...
    case 17: {
      ...
      e.player.message(
        '[§cNPC§f] §7Removed player: §b' + name + '§7 from adminlist!'
      );
...
function customGuiButton(e) {
  switch (e.buttonId) {
    case 1: {
      ...
      item.setLore([
        '§7Crystal holds a soul of a vendor inside',
        npc.getName(),
        '§8RightClick to set him free.',
      ]);
    ...
    case 8: {
      ...
      if (!rozmiar || isNaN(rozmiar) || rozmiar <= 0 || rozmiar > 7) {
        return e.player.message('[§cNPC§f] §7Size has to be between <1;7>');
      }
      if (!npc) {
        e.player.message('No npc!');
      }
      if (rozmiar == 7) {
        rozmiar = 1;
      } else {
    ...
    case 13: {
      ...
      if (!text) {
        return e.player.message('[§cNPC§f] §7Textbox empty!');
      }
    ...
    case 200: {
      ...
      if (skin && skin != display.getSkinUrl().split('/').pop()) {
        ...
        e.player.message(
          '[§cNPC§f] §7Changed skin to: §bhttps://i.imgur.com/' + skin
        );
      }
      return gui2(e.player);
    }
