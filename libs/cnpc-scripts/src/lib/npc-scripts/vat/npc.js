...
function dialogOption(e) {
  ...
  if (opt == 0) {
    return e.player.message(
      '[§cPodatnik§f] §7Aktualny stan zebranych podatków vat: §e' +
        parseInt(data.get('MMVat'))
    );
  } else if (opt == 1) {
    var kasa = parseInt(data.get('MMVat'));
    if (e.player.getName() != vname) {
      return e.player.message('[§cPodatnik§f] §7Nie jesteś: ' + vname);
    }
    if (kasa == 0) {
      return e.player.message(
        '[§cPodatnik§f] §7Nie ma żadnych knutów do wypłacenia!'
      );
    }
    data.put('MMVat', 0);
