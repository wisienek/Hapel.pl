...
function gui2(e) {
  ...
  gui.addLabel(
    10,
    '§7WOW, znalazłeś mnie tak szybko!\nChyba ty zostaniesz nowym mistrzem chowanego.\nNo nic, jak mówiłem dziele się swoimi zdobyczami z mistrzostw!\nNie wiem do czego ten srebrny pieniążek, ale wygląda podejrzanie. Możesz się za mnie dowiedzieć!',
    70,
    70,
    120,
    20
...
function customGuiButton(e) {
  switch (e.buttonId) {
    case 1: {
      ...
      if (niegor == 'szuka' || niegor == 'znalazł') {
        return e.player.message(
          '[§dNiegor§f] §7Ty szukasz już albo znalazłeś!'
        );
      }
      e.player.message(
        '[§dNiegor§f] §e**Zaśmiał się cichym głosikiem i zaraz zniknął!**'
      );
      e.player.closeGui();
      niegor = 'szuka';
    ...
    case 3: {
      var data = e.player.getStoreddata();
      var niegor = data.get('niegor');
      if (!niegor || niegor == 'znalazł') {
        return e.player.message(
          '[§dNiegor§f] §7Ty szukasz już albo znalazłeś!'
        );
      }

      var pieniążek = e.player.world.createItem(
      ...
      e.API.executeCommand(
        e.player.world,
        'dcdpm Przesladowca Gracz ' +
          e.player.getName() +
          ' zakończył questa i odebrał nagrodę!'
      );
      randLock(e);
      return e.player.closeGui();
    }
