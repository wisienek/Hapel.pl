...
function gui2(player, namioty, nr) {
  ...
  if (!nr) {
    ...
    gui.addLabel(
      2,
      '§1Koszt wynajmu z rezerwacją: §c20+20 knutów',
      44,
      90,
      160,
      20
    ...
    gui.addLabel(
      7,
      '§1Zwrot kosztów za oddany klucz: §c3 knuty',
      37,
      150,
      160,
      20
...
function gui4(player, namioty, selected) {
  ...
  if (!selected) {
    ...
    gui.addLabel(
      2,
      '§1Przedłużenie rezerwacji o 2 dni: §c10 knutów',
      30,
      90,
      160,
      20
...
function customGuiButton(e) {
  switch (e.buttonId) {
    ...
    case 3: {
      ...
      if (!klucze) {
        return e.player.message(
          '[§cNamioty§f] §7Nie masz żadnych kluczy do zwrotu!'
        );
      }
    ...
    case 4: {
      ...
      if (!nr || isNaN(nr)) {
        return e.player.message(
          '[§cNamioty§f] §7Coś poszło nie tak, nie znaleziono numeru namiotu!'
        );
      }
      if (takeMoney(e.player, false, 20)) {
        ...
        gui1(e.player);
        return e.player.message(
          '[§cNamioty§f] §7Zarezerwowano namiot nr.§e' + nr
        );
      } else {
        return e.player.message(
          '[§cNamioty§f] §7Nie masz wystarczającej gotówki! Trzymaj kasę na toolbarze!'
        );
      }
    }
    case 5: {
      ...
      if (takeMoney(e.player, false, 30)) {
        var chest = e.player.world
          .getBlock(924, 84, 1292)
          .getContainer()
          .getItems();
        if (!chest) {
          return e.player.message('[§cNamioty§f] §7Nie znaleziono kluczy!');
        }
        var key = chest[nr - 1];
        if (key.getName().indexOf('air') > -1) {
          return e.player.message('[§cNamioty§f] §7Nie znaleziono klucza!');
        }
        key.setStackSize(5);
    ...
    case 6: {
      ...
      if (lista.length == 0) {
        return e.player.message('[§cNamioty§f] §7Nie masz żadnych rezerwacji!');
      }
    ...
    case 12: {
      ...
      gui1(e.player);
      return e.player.message(
        '[§cNamioty§f] §7Zrezygnowano z rezerwacji namiotu nr.' + nr
      );
    }
  }
}
    ...
    case 2: {
      ...
      for (var i = 0; i < 9; i++) {
        if (items[i].getDisplayName() == e.selection[0]) {
          ...
          e.player.message(
            '[§cNamioty§f] §7Oddano klucz: §e' +
              e.selection[0] +
              ' §7Zwrot za wynajem: §e3 knuty'
          );
          odd = true;
      ...
      if (!odd) {
        e.player.message(
          '[§cNamioty§f] §7Musisz trzymać kluczyk na toolbarze!'
        );
      }
