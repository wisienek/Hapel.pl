  ...
  if (timeElapsed(last, '24hrs')) {
    ...
    if (x) {
      e.player.world.broadcast(
        '[§cPlecaki§f] §7Stworzono kopię zapasową plecaków!'
      );
    }
    ...
    last =
      ...
      date.getUTCMinutes();
    e.player.message(
      '[§cNPC§f] §7Ostatnia kopia została wykonana: §b' +
        last +
        '§7. Można robić co 24h'
    );
  }
}
function timeElapsed(e, t) {
  if (!t) {
    t = '10sec';
  }
  if (!e) return !1;
  switch (
    (t = {
      length: parseInt(t),
      type: t.split(parseInt(t))[1].toLocaleLowerCase(),
      dnow: Date.now(),
    }).type
  ) {
    case 'sec':
