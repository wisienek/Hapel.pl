...
function interact(e) {
  ...
  if (mainh.getName().indexOf('wygaszacz') > -1) {
    ...
    for (var i = 0; i < keys.length; i++) {
      e.player.message(
        '[§cPunkty§f] §a' + keys[i] + ' §4: §b' + wdata.get(keys[i])
      );
    }
...
function gui1(e) {
  ...
  var items = [
    e.player.world.createItem('hapeladdons:kartaczarodziei_common', 0, 1),
    ...
    var component = gui
      .addTexturedRect(
        ...
        item.getName().slice(0, item.getName().indexOf(':')) +
          ':textures/items/' +
          id[item.getName().slice(item.getName().indexOf(':') + 1)] +
          '.png',
        x + 1 + i * 30,
        y + 1,
        256,
      ...
      .setHoverText(['§7Ilość wymaganych pkt:', '§c' + pkt]);
...
function customGuiButton(e) {
  ...
  p = parseInt(p) || 0;
  if (p <= 0) {
    return e.player.message('[§cKarty§f] §7Masz za mało punktów');
  }
  ...
  switch (e.buttonId) {
    case 110: {
      if (p >= 2) {
        ...
        return e.player.message(
          '[§cKarty§f] §7Zakupiono kartę typu §acommon §7!'
        );
      } else {
        return e.player.message(
          '[§cKarty§f] §7Masz za mało punktów aby zakupić tę kartę!'
        );
      }
    }
    case 111: {
      if (p >= 10) {
        ...
        return e.player.message(
          '[§cKarty§f] §7Zakupiono kartę typu §auncommon §7!'
        );
      } else {
        return e.player.message(
          '[§cKarty§f] §7Masz za mało punktów aby zakupić tę kartę!'
    ...
    case 112: {
      if (p >= 50) {
        ...
        return e.player.message(
          '[§cKarty§f] §7Zakupiono kartę typu §aancient §7!'
        );
      } else {
        return e.player.message(
          '[§cKarty§f] §7Masz za mało punktów aby zakupić tę kartę!'
    ...
    case 113: {
      if (p >= 150) {
        ...
        return e.player.message(
          '[§cKarty§f] §7Zakupiono kartę typu §arare §7!'
        );
      } else {
        return e.player.message(
          '[§cKarty§f] §7Masz za mało punktów aby zakupić tę kartę!'
    ...
    case 114: {
      if (p >= 2000) {
        ...
        return e.player.message(
          '[§cKarty§f] §7Zakupiono kartę typu §alegendary §7!'
        );
      } else {
        return e.player.message(
          '[§cKarty§f] §7Masz za mało punktów aby zakupić tę kartę!'
