...
new Gui(
  ...
  function (e) {
    ...
    for (var i = 0; i < regKeys.length; i++)
      regiony[regKeys[i]].rented
        ? rented.push('§c' + regKeys[i])
        : rented.push('§a' + regKeys[i]);
    ...
    */
    ...
    gui
      .addButton(id('removeRegion'), 'Usuń region', 95, 165, 80, 20)
      .setHoverText(['Zaznacz region do usunięcia i kliknij']);

    gui
      .addTextField(id('textRegionPrice'), -15, 165, 20, 20)
      .setHoverText(['Wpisz tutaj cenę regionu']);
    gui
      .addTextField(id('textRegionName'), 10, 165, 80, 20)
      .setHoverText(['Wpisz tutaj nazwę regionu']);
    ...
    case id('addRegion'):
      ...
      if (!t || t.length == 0 || (!c && c != 0))
        return e.player.message(
          '[§cRenter§f] §7Nie wpisano nazwy lub ceny regionu!'
        );
