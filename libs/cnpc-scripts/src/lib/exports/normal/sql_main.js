...
var urlp =
  'jdbc:mysql://185.242.134.76:3306/em411_cnpc?useUnicode=true&characterEncoding=utf8';
...
function removeEliksir(nazwa) {
  if (!nazwa) return { error: 'No arguments: nazwa (varchar)' };

  var query =
    "DELETE FROM Eliksiry WHERE nazwa='" + escapeString(nazwa) + "'; ";
  return sqlPut(query);
}
...
function addSkladnik(nazwa, typ, cena, ilosc, jednostka, dostępny) {
  ...
  var query =
    'INSERT INTO Skladniki (nazwa, typ, cena, ilosc, jednostka, dostępny) VALUES ("' +
    escapeString(nazwa) +
    '", "' +
    typ +
    '", "' +
...
function removeSkladnik(nazwa) {
  ...
  var query =
    "DELETE FROM Skladniki WHERE nazwa='" + escapeString(nazwa) + "'; ";
  return sqlPut(query);
}
...
function getSkladnik(args) {
  ...
  if (args) {
    if (args.typ) {
      query += 'WHERE typ="' + args.typ + '" ';
    } else if (args.nazwa) {
      query += "WHERE nazwa LIKE '%" + escapeString(args.nazwa) + "%' ";
    }
  }
...
function getOczekujace(args) {
  ...
  if (args) {
    ...
    if (args.gracz) {
      x.push('gracz="' + escapeString(args.gracz) + '" ');
    }
    if (args.discord) {
      x.push('discord="' + args.discord + '" ');
    }
    if (args.eliksir) {
      x.push("eliksir LIKE '%" + escapeString(args.eliksir) + "%' ");
    }
    if (args.cena) {
      x.push('cena > ' + args.cena + ' ');
    }
    if (args.id) {
      x.push('id="' + args.id + '" ');
    }
    if (args.weryfikowane || args.weryfikowane == false) {
      x.push('weryfikowane=' + args.weryfikowane + ' ');
    }
    if (args.odebrane == true || args.odebrane == false) {
      x.push('odebrane = ' + args.odebrane + ' ');
    }
...
function updateOczekujace(id, args) {
  ...
  if (args.odebrane || args.odebrane == false) {
    x.push('odebrane = ' + args.odebrane + ' ');
  }
  if (args.data_odebrania) {
    x.push('data_odebrania="' + args.data_odebrania + '" ');
  }
  if (args.weryfikowane) {
    x.push('weryfikowane = ' + args.weryfikowane + ' ');
  }
  if (args.ile) {
    x.push('pile = ' + args.ile + ' ');
  }
  if (args.data) {
    x.push('pdata = "' + args.data + '" ');
  }
...
function addOczekujace(
  ...
  var query =
    'INSERT INTO Oczekujace (' + args[0] + ') VALUES (' + args[1] + '); ';

  return sqlPut(query);
}

function removeOczekujace(id) {
  if (!id) return { error: 'No arguments: id (varchar)' };
...
function addPrzepis(args) {
  ...
  var query =
    'INSERT INTO przepisy(' + keys.join(', ') + ') VALUES (' + vals + ');';
...
function getPrzepis(args) {
  ...
  if (args && args.eliksir) {
    opt.push("eliksir='" + args.eliksir + "' ");
  }
  if (args && args.autor) {
    opt.push("autor='" + args.autor + "' ");
  }
  if (args && args.item) {
    opt.push("item='" + args.item + "' ");
  }
...
function addCennik(args) {
  ...
  var query =
    'INSERT INTO cennik(' + keys.join(', ') + ') VALUES (' + vals + ');';
...
function getCennik(args) {
  ...
  if (args.nazwa) {
    opt.push("nazwa='" + args.nazwa + "' ");
  }
  if (args.cena) {
    opt.push('cena ' + args.cena + ' ');
  }
  if (args.sale) {
    opt.push("sale >= '" + args.dostępne + "' ");
  }
  if (args.msale) {
    opt.push("msale >= '" + args.msale + "' ");
  }
  if (args.tagi) {
    opt.push("tagi LIKE '%" + args.tagi + "%' ");
  }
  if (args.autor) {
    opt.push("autor='" + args.autor + "' ");
  }
  if (args.monly || args.monly == false) {
    opt.push("monly='" + args.monly + "' ");
  }
  if (args.dostępne || args.dostępne == false) {
    opt.push("dostępne='" + args.dostępne + "' ");
  }
  if (args && Object.keys(args).indexOf('wer') > -1) {
    opt.push("wer='" + args.wer + "' ");
  }
  if (args && opt.length > 0) {
    opt = opt.join(' AND ');
