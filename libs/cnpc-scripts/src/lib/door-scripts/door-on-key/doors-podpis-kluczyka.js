  ...
  if (mainh.getName() == 'minecraft:written_book') {
    ...
    if (tytul.length == 2) {
      ...
      return e.player.message(
        'Już mam temat książki: ' +
          tytul +
          '§f, jeżeli chcesz go usunąć kliknij na mnie pustą ręką'
      );
    }
    ...
    e.player.message(
      'Aby dodać opis do kluczyka musisz mieć podpisaną książkę odpowiednim textem - brana pod uwagę jest jedynie pierwsza linijka! Najpierw kliknij nią na mnie ppm, a potem kluczykiem'
    );
    tytul = '§e';
    return;
  }
