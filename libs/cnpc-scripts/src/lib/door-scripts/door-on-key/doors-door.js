...
function interact(e){
    ...
    if(mainh!=null){
        ...
        if(mainh.getDisplayName()=="Kluczyk do drzwi"){
            ...
            if(claim==0){
                if(mainh.getLore()[0]=="§a**Oryginalny kluczyk**"||mainh.getLore()[0]=="§c**Podrobiony kluczyk**"){
                    e.setCanceled(true);
                    e.player.message("Kluczyk nie pasuje do tych drzwi!");
                    return;
                }else if(mainh.getLore()[0].toLowerCase()=="**nieprzypisany kluczyk**"){
                    if(toggle==0){
                        toggle=1;
                        e.block.setOpen(true);
                        mainh.setStackSize(6);
                        mainh.setLore(["§a**Oryginalny kluczyk**","§0"+kordy,"§0"+e.player.getUUID()]);
                ...
                if(mainh.getLore().length>2){
                    if(mainh.getLore()[2].indexOf(e.block.getStoreddata().get("DOwner"))>-1 && mainh.getLore()[1].indexOf(kordy)>-1){
                        if(toggle==1){
                            e.block.setOpen(false);
                            toggle=0;
                    ...
                    if(mainh.getLore()[1]=="§cKopia"){
                        mainh.setLore(["§a**Oryginalny kluczyk**","§0"+kordy,"§0"+e.block.getStoreddata().get("DOwner")]);
                        if(mainh.getStackSize()<2){mainh.setStackSize(2);}
                    }
                    e.setCanceled(true);
                    e.player.message("Nie masz odpowiedniego kluczyka!");
                }
            }
        }else if(mainh.getDisplayName()=="Wytrych" && (mainh.getLore()[0].toLowerCase()=="**dobrej jakości wytrych**"||mainh.getLore()[0].toLowerCase()=="**dobrej jakości magiczny wytrych**")){
            if(hardness>=80 && hardness!=100 && mainh.getLore()[0].toLowerCase()!="**dobrej jakości magiczny wytrych**"){
                e.setCanceled(true);
                return e.player.message("Drzwi nie da się otworzyć normalnym wytrychem");
            }else
            if(hardness==100){
                e.setCanceled(true);
            ...
            if(wytrych()==true){
                ...
                if(times>1){
                    times-=1;
                    mainh.setLore([mainh.getLore()[0],times+"/"+mainh.getLore()[1].split("/")[1]]);
                }else{
                    mainh.setStackSize(mainh.getStackSize()-1);
                }
                return;
            }
        }else if(mainh.getDisplayName()=="Magiczny zamek" && mainh.getLore()[0].toLowerCase()=="**prawdziwy zaklęty zamek**"){
            ...
            mainh.setStackSize(mainh.getStackSize()-1);
            e.player.sendNotification("Zaklęto drzwi","Pomyślnie nałożono kłudkę o mocy: "+ hardness1, 2);
            savedoor(e.player, kordy, loaddoor(kordy), claim, hardness1);
            hardness = hardness1;
            return;
        }else if(mainh.getDisplayName()=="info"){
            e.setCanceled(true);
            return e.player.message("claim: "+claim+", Zabezpieczenie: "+hardness+", Kordy: "+kordy+", uuid: "+e.block.getStoreddata().get("DOwner"));
        }else if(mainh.getDisplayName()=="lock-break" && mainh.getLore()[0]=="**Admin lock breaker**" && claim==1){
            savedoor(e.player,kordy,loaddoor(kordy),0,0);
            return e.player.sendNotification("Lock-Breaker","Z powodzeniem złamano zabezpieczenia drzwi",2);
        }else{
            e.setCanceled(true);
            return;
        }
