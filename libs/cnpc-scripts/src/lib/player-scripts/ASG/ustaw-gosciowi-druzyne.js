function attack(e) {
  ...
  if (lore.length == 0) {
    return mainh.setLore(['Galeoniarze']);
  } else {
    var teams = data.get('teams');
    var i = teams.indexOf(lore[0]);
    if (i == -1) {
      return e.player.message('coś nie tak');
    }
...
function interact(e) {
  ...
  var lore = mainh.getLore();
  if (lore.length == 0) {
    return e.player.message('w 1 linijce opisu nie ma nazwy teamu');
  }
  var target;
  if (e.type == 1 && e.target.getTypeName() != 'CustomNpc') {
    target = e.target;
  } else {
    return e.player.message('Nie znaleziono gracza ;/');
  }
  if (target) {
    ...
    return e.player.message(
      'Dodano ' + target.getName() + ' do drużyny ' + team.getDisplayName()
    );
  }
}
