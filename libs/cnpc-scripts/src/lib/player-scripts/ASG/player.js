function damaged(e) {
  if (e.damageSource.isProjectile() == true) {
    ...
    if (
      (mainh.getDisplayName() == '§cMagiczny łuk' ||
        mainh.getDisplayName() == '§cMagiczny Pistolecik') &&
      temp.get('mecz') == 'start'
    ) {
      ...
      if (teamp && teamp != teama) {
        ...
        powiadom(
          e.player.world,
          '[§cASG§f] §e' +
            atakuje.getDisplayName() +
            ' §b(' +
            teama.getDisplayName() +
            ')§7 zdobył punkt przez trafienie: §e' +
            e.player.getDisplayName() +
            ' §b(' +
            teamp.getDisplayName() +
            ')'
        );
