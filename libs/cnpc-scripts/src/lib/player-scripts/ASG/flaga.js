...
function init(e) {
  ...
  if (
    !data12.get(data12.get('team') + '-f') ||
    data12.get(data12.get('team') + '-f') == 0
  ) {
...
function interact(e) {
  ...
  if (data1.get('mecz') == 'stop') {
    return e.player.message(
      '[§cASG§f] §7Mecz nie wystartował, flaga drużyny: §b' + data11.get('team')
    );
  }
  if (mainh.getDisplayName().indexOf('Reset Flagi') > -1) {
    return;
  }
  if (mainh.getDisplayName().indexOf('Ustawianie flag') > -1) {
    return;
  }
  if (!bteam) {
    return e.player.message('[§cASG§f] §7Flaga nie ma ustawionego teamu.');
  }
  if (!team) {
    return e.player.message(
      '[§cASG§f] §7Nie jesteś w żadnej drużynie, nie możesz przejądź flagi!'
    );
  }
  if (team.getDisplayName() == bteam) {
    if (
      mainh.getDisplayName().indexOf('Flaga drużyny') > -1 &&
      mainh.getDisplayName().indexOf(team) == -1
    ) {
      ...
      powiadom(
        ...
        '[§cASG§f] §e' +
          ...
          '§7 Zdobył §c15 punktów§7 dla: §b' +
          team.getDisplayName() +
          '§7 za przejęcie flagi §b' +
          mainh.getDisplayName().split(':')[1] +
          '§7 !'
      );
