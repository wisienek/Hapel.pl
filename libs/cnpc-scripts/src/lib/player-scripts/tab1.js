...
function interact(e) {
    try {
        ...
        if (mainh.getName().indexOf("hapeladdons:czapka") > -1 || mainh.getName().indexOf("czaszka") > -1 || mainh.getName().indexOf("dynia") > -1 || mainh.getName().indexOf("tiara") > -1) {
            ...
            if ( lastO && !Permission.has(e.player.getName(), "litebans.ban") ) {
            ...
            if ( lore.length >= 2 )
                if (!Permission.has(e.player.getName(), "litebans.ban") && e.player.getUUID() != lore[1].replace(/§./g, ""))
            ...
            if ( e.player.isSneaking() ) {
                ...
                gui.addLabel(5, "§2§l?", 195, 90, 5, 15)
                    .setHoverText(["§cPomoc", "§7Aby wpisać kolor lub inne specjalne kodowanie", "§7Daj znak §l&§r§7 i potem znak kodu np. kolor §55"]);
                ...
                gui.addButton(4, "§aStwórz kopię", 90, 200, 80, 20)
                    .setHoverText(["§cInfo", "§7Raz dziennie możesz zapisać plecak", "§7Pozwala to na późniejsze przywrócenie", "§7W razie kłopotów", "§7Ostatnia kopia: §a" + data]);

                gui.addTextField(11, 70, 140, 120, 15)
                    .setHoverText(["§cInfo", "§7Wpisz tutaj dokładny nick gracza", "§7Aby przekazać mu prawa do plecaka."]);
                
                gui.addButton(10, "§7Zmień prawa", 90, 160, 80, 20).
                    setHoverText(["§cInfo", "§7Wpisz powyżej dokładny nick gracza", "§7Aby przekazać mu prawa do plecaka."]);

                return e.player.showCustomGui(gui);
            }

            if ( e.player.world.getAllPlayers().length > 50 && !Permission.has(e.player.getName(), "litebans.ban") ) 
            ...
            if ( !inventory ) {
                ...
                if (lore.length == 0) {
                    ...
                    saveBag(uuid, e.player.getName(), JSON.stringify([]), nrp);
                ...
                if (!plecak.nazwa || !plecak.uuid) {
                    var t = updateName(plecak.id, e.player.getName(), uuid);
                    if (!t) {
                ...
                if (lore[1].indexOf(uuid) > -1 || Permission.has(e.player.getName(), "litebans.ban")) {
...
function toss(e) {
    try {
        ...
        if( 
            ...
            dname.toLowerCase().indexOf("stick") > -1 
        ) {
            var confirm = e.player.getTempdata().get("confirmThrow");
            if( !confirm ){
