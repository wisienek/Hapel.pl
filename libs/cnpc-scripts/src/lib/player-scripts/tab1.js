...
function interact(e) {
  try {
    ...
    if (e.player.isSneaking()) {
      ...
      if (riders.length > 0) return e.player.clearRiders();
    }
    ...
    if (!mainh || mainh.getName().toLowerCase().indexOf('air') > -1) return;

    if (
      mainh.getName().indexOf('hapeladdons:czapka') > -1 ||
      mainh.getName().indexOf('czaszka') > -1 ||
      mainh.getName().indexOf('dynia') > -1 ||
      mainh.getName().indexOf('tiara') > -1
    ) {
      ...
      mainh.getName() == 'minecraft:spawn_egg' &&
      mainh.getDisplayName() == '§7Czekoladowa Żaba'
    ) {
      e.setCanceled(true);
      ...
      var seller = e.player.world.spawnClone(
        pos.getX(),
        pos.getY(),
        pos.getZ(),
        9,
      ...
      mainh.getDisplayName().indexOf('Skrystalizowany sprzedawca') > -1
    ) {
      e.setCanceled(true);
      ...
      if (!nbt.getInteger('Żywa')) {
        ...
        if (e.type == 1 && e.target.getName().indexOf(name) > -1) {
          ...
          if (
            cnbt.getString('master') == e.player.getUUID() ||
            cnbt.getString('master_name') == e.player.getName()
          ) {
            e.target.despawn();
      ...
      if (lastO && !Permission.has(e.player.getName(), 'maxbans.ban')) {
        ...
        if (Date.now() - lastO < cld * 1000)
          return e.player.message(
            '[§cPlecak§f] §7Możesz otwierać plecak co: §c' + cld + '§7 sekund!'
          );
      }
      ...
      if (lore.length >= 2)
        if (
          ...
          e.player.getUUID() != lore[1].replace(/§./g, '')
        )
      ...
      if (e.player.isSneaking()) {
        ...
        gui.addLabel(
          6,
          '§2Plecak numer: §e' + lore[0].replace(/§./g, ''),
          100,
          75,
          90,
          15
      ...
      if (!inventory) {
        ...
        if (
          lore[1].indexOf(e.player.getUUID()) > -1 ||
          Permission.has(e.player.getName(), 'maxbans.ban')
        ) {
          if (mainh.getDisplayName().split(' ')[0] == '§cŚredni') {
            inventory = new JIb(
              '\u00A74Średni Plecak \u00A7e' +
                mainh.getLore()[0].split('§0')[1],
              true,
              27
            );
          } else if (mainh.getDisplayName().split(' ')[0] == '§cDuży') {
            inventory = new JIb(
              '\u00A74Duży Plecak \u00A7e' + mainh.getLore()[0].split('§0')[1],
              true,
              36
            );
            ...
            inventory = new JIb(
              '\u00A74Mały Plecak \u00A7e' + mainh.getLore()[0].split('§0')[1],
              true,
              18
            );
          }
      ...
      return e.player.world
        .getStoreddata()
        .put('wizytówki', JSON.stringify(wdata));
      ...
      if (Date.now() > data) {
        ...
        if (!isNaN(parseFloat(czas))) {
          sudo(
            e.API,
            e.player.getName(),
            'do Eliksir będzie trwał przez: ' + czas + ' minut'
          );
      ...
      mainh.getDisplayName().toLowerCase().indexOf('paczka kart czarodziejów') >
      -1
    ) {
      var nbt = mainh.getNbt();
      ...
      for (var i = 0; i < ammount; i++) karta(e, karty);
      ...
      if (e.player.getName() != 'Przesladowca')
        sudo(
          e.API,
          e.player.getName(),
          'me Otwiera: ' + wersja + ' paczka kart czarodziejów'
        );

      return e.player.message(
        '[§cKarty§f] §7Otworzno paczkę kart czarodziejów!'
      );
        ...
        if (e.type == 2) {
          if (e.target.getName() == 'minecraft:cauldron') {
            ...
            try {
              ...
              var scripts = nbt.getList(
                'Scripts',
                nbt.getListType('Scripts')
              )[0];
              var sl = [];
              var requiredScripts = [
                'kociolek.js',
      ...
      if (
        miotly.indexOf(
          mainh.getDisplayName().split(' ').join('').split('§f')[1]
        ) > -1
      ) {
        ...
        for (var x = 0; x < lore.length; x++) {
          lore1.push(lore[x]);
          if (
            lore[x].toLowerCase().indexOf('oryginalności') > -1 ||
            lore[x].toLowerCase().indexOf('oryginalnosci') > -1
          ) {
            znaczek = true;
          }
        ...
        if (Math.floor(Math.random() * 1000) > w) {
          if (wytrzymalosc) {
            ...
            if (wyt == 0) {
              ...
              var pmiotla = e.player.world.createItem(
              ...
              wyt--;
              lore1[wytrzymalosc] =
                lore1[wytrzymalosc].split(' ')[0] + ' ' + wyt;
              mainh.setLore(lore1);
              return e.player.message(
  ...
  try {
    ...
    if (!pdata.has('dc')) {
      ...
      if (dcid && dcid.length > 0) {
        var test = sqlGet(
          "SELECT * FROM discords WHERE dcdid='" + dcid + "'; "
        );
        ...
        if (test.result.length > 0) {
          ...
          if (data1[e.player.getName()]) return;
          ...
          var test1 = sqlPut(
            "UPDATE discords SET data='" +
              ...
              "' WHERE serial=" +
              test.result[0].serial
          );
          if (test1.error) throw test1.error;
        } else {
          var test1 = sqlPut(
...
function toss(e) {
  try {
    ...
    if (
      miotly.indexOf(dname.split(' ').join('').split('§f')[1]) > -1 ||
      dname.toLowerCase().indexOf('knut') > -1 ||
      dname.toLowerCase().indexOf('sykl') > -1 ||
      dname.toLowerCase().indexOf('galeon') > -1 ||
      dname.toLowerCase().indexOf('różdżka') > -1
    ) {
    ...
    if (ents.length > 0) {
      if (ents[0].getType() == 1) {
        ...
        var stacks = 1;
        if (e.player.isSneaking()) stacks = e.item.getStackSize();
...
function getEmpty(npc) {
  ...
  if (role.getType() == 1) {
    for (var x = 0; x <= 17; x++) {
      ...
      if (slot.getName().indexOf('air') > -1) return x;
    }
  }
  return false;
}
...
function karta(e, karty) {
  try {
    ...
    if (chance >= 985) {
      rarity = 'legendary';
    } else if (chance < 985 && chance >= 935) {
      rarity = 'rare';
    } else if (chance < 935 && chance >= 835) {
      rarity = 'ancient';
    } else if (chance < 835 && chance >= 500) {
      rarity = 'uncommon';
    } else if (chance < 500) {
      rarity = 'common';
    }
    ...
    var item = e.player.world.createItem(
      'hapeladdons:kartaczarodziei_' + rarity,
      0,
      1
    );
    ...
    var lore = [
      '§aRzadkość: §d' + rarity,
      '§aNumer: §f' + karty[rarity][id].numer,
    ].concat(karty[rarity][id].opis);
    item.setLore(lore);
    ...
    if (x.wyslane) nbt.setString('owner', e.player.getName());
  } catch (er) {
  ...
  try {
    switch (e.buttonId) {
      ...
      case 4: {
        ...
        if (Date.now() - data > 86400000) {
          ...
          if (r === true) {
            ...
            return e.player.message(
              '[§cPlecaki§f] §7Poprawnie zapisano plecak!'
            );
          }
      ...
      case 10: {
        ...
        if (!nick) return e.player.message('[§cPlecak§f] §7Nie wpisano nicku!');
        ...
        e.player.message(
          '[§cPlecak§f] §7Przekazano prawa plecaka dla: §a' + player.getName()
        );
        e.player.closeGui();
