function interact(e) {
  ...
  if (
    mainh.getName() == 'variedcommodities:letter' &&
    mainh.getDisplayName().indexOf('Przepustka do pokoju VIP') > -1
  ) {
    var lore = mainh.getLore();
    if (lore.length < 2) {
      return e.player.message('Przepustka bez właściciela');
    } else {
      if (lore[1].indexOf(e.player.getName()) == -1) {
        return e.player.message(
          '[§cGringott§f] §7Przepustka należy do: ' +
            lore[1] +
            '§7. Nie możesz jej użyć.'
        );
      }
    }
    ...
    for (var x = 0; x < players.length; x++) {
      if (players[x].getName() == e.player.getName()) {
        ...
        return e.player.message(
          '[§cGringot§f] §7Przeniesiono do VIP-roomu gringota!'
        );
      }
    }
