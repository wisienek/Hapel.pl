  ...
  if (mainh.getDisplayName().indexOf('Plecak.') > -1) {
    ...
    if (!inventory) {
      ...
      if (plecaki) {
        ...
        if (!plecak) {
          return e.player.message('[§cPleak§f] §7Coś poszło nie tak!');
        }
      } else {
        if (lore.length > 0) {
          return e.player.message('[§cPleak§f] §7Nie można zapisać plecaka');
        }
        mainh.setLore(['§01']);
        plecaki = { nr: 1, 1: [] };
        nrp = 1;
      ...
      if (
        ...
        e.player.getGamemode() == 1
      ) {
        if (mainh.getDisplayName().split(' ')[0] == '§cŚredni') {
          inventory = new JIb(
            '\u00A74Średni Plecak \u00A7e' + mainh.getLore()[0].split('§0')[1],
            true,
            18
          );
