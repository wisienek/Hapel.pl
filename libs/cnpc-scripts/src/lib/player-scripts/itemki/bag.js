...
function interact(e) {
  ...
  if (mainh.getDisplayName().indexOf('§cPlecak') > -1) {
    ...
    if (lore.length == 0) {
      return e.player.message('[§cPlecak§f] §7Niepoprawny plecak!');
      ...
      return e.player.message(
        '[§cPlecak§f] §7Dodano id:§e' + p + '§7  do plecaczka.'
      );
    }

    if (e.player.getStoreddata().get('plecak') == 1) {
      return e.player.message('[§cPlecak§f] §7Plecak jest nadal otwierany!');
    }
    ...
    if (lore[0].indexOf('Zamknięty') > -1) {
      if (e.player.getStoreddata().get('plecak') == 2) {
        return e.player.message('[§cPlecak§f] §7Masz już otwarty plecak!');
      }
      if (d.indexOf(lore[1].split('§0')[1]) > -1) {
        return e.player.message(
          '[§cPlecak§f] §7Ten plecak jest już gdzieś otwarty!'
        );
      }
      ...
      if (d.indexOf(lore[1].split('§0')[1]) > -1) {
        d.splice(d.incexOf(lore[1].split('§0')[1]), 1);
      }
      data.put('plecaki1', JSON.stringify(d));
      e.player.getStoreddata().put('plecak', 1);
      ...
      return e.player.message('[§cPlecak§f] §7Błąd, niepoprawny lore[0]!');
    }
  }
}
    ...
    for (var x = 9; x < 36; x++) {
      if (inv.getSlot(x).getName() != 'minecraft:air') {
        ...
        if (item.getDisplayName() == '§cPlecak' && loopbag == false) {
          ...
          e.player.message(
            '[§cPlecak§f] §7Nie możesz trzymać plecaków w plecakach, wyrzucono na ziemię (brak miejsca w toolbarze)!'
          );
...
function toss(e) {
  if (e.item.getDisplayName() == '§cPlecak') {
    ...
    if (lore.length > 0) {
      if (
        lore[0].indexOf('Otworzony') > -1 ||
        lore[0].indexOf('Otwarty') > -1
      ) {
        e.setCanceled(true);
