function interact(e) {
  ...
  if (bags.length > 0) {
    var test = popraw(
      escapeString(bags[0].itemy.replace(/    /g, '').replace(/\n/g, '')),
      bags[0].id
    );
    if (!test) {
      return e.player.message('Coś poszło nie tak na pierwszym!');
    }
    for (var i = 1; i < bags.length; i++) {
      var t = popraw(
        escapeString(bags[i].itemy.replace(/    /g, '').replace(/\n/g, '')),
        bags[i].id
      );
      if (!t) {
...
function popraw(itemy, id) {
  ...
  try {
    conn = DriverManager.getConnection(urlp, login, password);
    var query =
      "UPDATE main SET itemy='" + itemy + "' WHERE id = '" + id + "' ;";
    var st = conn.createStatement();
