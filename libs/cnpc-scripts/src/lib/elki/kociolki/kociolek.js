...
function baseGui(b) {
  try {
    ...
    if (!KOCIOLEK) throw 'Brak KOCIOLEK!';
    ...
    b
      ...
      : gui.setBackgroundTexture('customnpcs:textures/gui/elki.png');
    ...
    gui
      ...
      .setHoverText([
        '§7Przełącz kociołek',
        '§7Aktualny kociołek: §a' + (elki.id || 'Główny'),
        '§7' + (sdata.get('kociolek') || 'Kociołek cynowy, Rozmiar 2'),
      ]);
...
function gui6(player) {
  ...
  gui.addLabel(
    156,
    '§3Aktualna temperatura: §b' + elki.current.temp + ' §3°C',
    20,
    100,
    100,
    20
  ...
  if (!b) {
    ...
    if (skladniki.error) {
      return e.player.message(
        '[§cDebugger§f] §7Error, napisz do administracji! -' + skladniki.error
      );
    }
    ...
    for (var i = 0; i < skladniki.length; i++) {
      if (
        search &&
        skladniki[i].nazwa.toLowerCase().indexOf(search.toLowerCase()) > -1
      ) {
        if (skladniki[i].dostępny == true) {
          lista1.push(
            skladniki[i].nazwa + '  [' + skladniki[i].jednostka + ']'
          );
        }
      } else if (!search) {
        if (skladniki[i].dostępny == true) {
...
function gui11(player, searched) {
  ...
  data = data.result;
  var lista = [];
  for (var i = 0; i < data.length; i++) lista.push(data[i].nazwa);
  ...
  if (!eliksir) {
    ...
    for (var i = 0; i < items.length; i++) {
      if (items[i].getDisplayName() == '§eTajemniczy Eliksir') {
        ...
        if (lore.length >= 3)
          eliksiry.push(
            i + '. ' + lore[0] + ' §7§l/§r ' + lore[1] + ' §7§l/§r ' + lore[2]
          );
      }
    }
...
function customGuiSlot(e) {
  try {
    if (!KOCIOLEK) return print('Nie znaleziono egzekującego bloku!');
...
function customGuiButton(e) {
  ...
  try {
    ...
    switch (e.buttonId) {
      case 1: {
        /* kończ eliksir       */ gui2(e.player);
        break;
      }
      case 11: {
        /* nowy eliksir        */ gui3(e.player);
        break;
      }
      case 12: {
        //zapisz przepis:
      ...
      case 31: {
        /* wróć do menu główne */ gui1(e.player);
        break;
      }
      case 32: {
        //etykiety usuwanie
      ...
      case 41: {
        /* inokreacja          */ gui12(e.player);
        break;
      }
      case 42: {
        /* etykieter           */ gui13(e.player);
        break;
      }
      case 55: {
      ...
      case 101: {
        /* otwórz kociołek    */ gui6(e.player);
        break;
      }
      case 102: {
        /* dodaj składnik     */ gui7(e.player);
        break;
      }
      case 103: {
        /* pokaż listę kroków */ gui8(e.player);
        break;
      }
      case 104: {
        //zapisz do itemu
      ...
      case 202: {
        ...
        pisz(e.player, 'Wlał jakiś płyn do kociołka §7[' + x.ile + 'ml]');
        break;
      }
      case 203: {
        var x = handleAdd(e);
        ...
        var y = temp(e, parseInt(parseFloat(x.ile)));
        if (!y) {
          return;
        }
        pisz(e.player, 'Zwiększył temperaturę w kociołku o §7' + x.ile + '°C');
        break;
      }
      case 204: {
        var x = handleAdd(e);
        ...
        var y = temp(e, -1 * parseInt(parseFloat(x.ile)));
        if (!y) {
          return;
        }
        pisz(e.player, 'Zmniejszył temperaturę w kociołku o §7' + x.ile + '°C');
        break;
      }
      case 205: {
        var tmp = JSON.parse(KOCIOLEK.getStoreddata().get('elki')) || {};
        if (tmp.current.ignis == true) {
          return e.player.message(
            '[§cKociołek§f] §7Nie możesz podwójnie rozpalić ogniska ;0'
          );
        }
        KOCIOLEK.getTimers().forceStart(1, 90, true);
        tmp.current.ignis = true;
        KOCIOLEK.getStoreddata().put('elki', JSON.stringify(tmp));
        var x = handleAdd(e);
        if (!x) {
          return;
      ...
      case 206: {
        var tmp = JSON.parse(KOCIOLEK.getStoreddata().get('elki')) || {};
        if (tmp.current.ignis == false) {
          return e.player.message(
            '[§cKociołek§f] §7Już jest zgaszone palenisko!'
          );
        }
        KOCIOLEK.getTimers().stop(1);
        temp(e, -1 * tmp.current.temp);
        tmp.current.ignis = false;
        KOCIOLEK.getStoreddata().put('elki', JSON.stringify(tmp));
        var x = handleAdd(e);
        if (!x) {
          return;
      ...
      case 207: {
        ...
        pisz(e.player, 'Zamieszał w lewo §7' + x.ile + ' razy');
        break;
      }
      case 208: {
        var x = handleAdd(e);
        ...
        pisz(e.player, 'Zamieszał w prawo §7' + x.ile + ' razy');
        break;
      }
      case 209: {
        var x = handleAdd(e);
        ...
        pisz(e.player, 'Odstawił eliksir na §7' + x.ile + ' minut ');
        setWait(x.ile);
        break;
      }
      case 212: {
        var akcja = e.gui.getComponent(211).getText();
        if (!akcja) {
          return e.player.message('[§cKociołek§f] §7');
        }
        addToList('AC: ' + akcja, e.player);
        pisz(e.player, akcja);
        break;
      }
      case 301:
      ...
      case 305: {
        /* własny składnik */ gui10(e.player);
        break;
      }
      ...
      case 991: {
        /* powrót z dodawania składników */ gui3(e.player, true, true);
        break;
      }
      case 998: {
        var text = e.gui.getComponent(7).getText();
        return gui11(e.player, text);
      }
      case 999: {
        var text = e.gui.getComponent(7).getText();
...
function calculatePrice(player, skladnik) {
  ...
  elki = JSON.parse(elki) || {};
  if (!elki.current) return false;

  var price = elki.current.price || 0;
  var value =
    Math.ceil(
      (parseFloat(skladniki.cena) / parseFloat(skladniki.ilosc)) * skladnik.ile
    ) || 9999;
...
function temp(e, ile) {
  ...
  e.gui
    .getComponent(156)
    .setText('§3Aktualna temperatura: §b' + elki.current.temp + ' §3°C');
...
function handleAdd(e) {
  ...
  if (
    e.gui.getComponent(e.buttonId - 50) &&
  ...
  if (!co) return e.player.message('[§cEliksiry§f] §7brak guzika!');
...
function handleSkladnik(e) {
  ...
  if (elki.current && elki.current.skladnik) {
    ...
    if (elki.current.skladnik.akcje.indexOf(bttnText) > -1) return false;
...
function setWait(ile) {
  if (!ile || ile <= 0 || isNaN(ile)) return false;
...
function checkArray(ar, t) {
  ...
  for (var x = 0; x < sar.length; x++) if (sar[x] == t) i++;
