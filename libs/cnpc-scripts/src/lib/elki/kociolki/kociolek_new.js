    ...
    try {
        ...
        switch (e.scrollId) {
            case id("s-ingredient"): {
                ...
                var ile = parseFloat(e.gui.getComponent(id("in-quantity")).getText()).toFixed(2);
    ...
    try{
        ...
        switch ( e.buttonId ) {
            ...
            case id("b-goBack"): {
                ...
                if( current && current.showPrev )
                    return current.showPrev( e );

                break;
            }
            case id("b-ingredientType5"):{ return _GUI["eqIngredient"].show(e); }
            case id("b-continueBrewing"):  { return _GUI["continueBrewing"].show(e); }
            case id("b-inocreation"): { return _GUI["eliksirInocreation"].show(e); }
            case id("b-labelMaker"): { return _GUI["labelMaker"].show(e); }
            case id("b-swtichToCrucible"): { return _GUI["basicInputs"].show(e); }
            case id("b-goAddIngredient"): { return _GUI["ingredientSelector"].show(e); }
            case id("b-steps"): { return _GUI["stepList"].show(e); }
                ...
                case id("b-timeKeeper"):
            {
                ...
                addElement(e, (HANDLE_ADD.indexOf(text) > -1), text || "[NIEZ NALEZIONO AKCJI!!!!]");
                return;
            }
            case id("b-customAction"): {
                ...
                var ndata = KOCIOLEK.getStoreddata();
                var elki = addToList( JSON.parse(ndata.get('elki')), "AC: " + akcja, e.player); 
...
function updateKociolekDependencies(e) {
    ...
    try {
        if( !KOCIOLEK ) 
            throw "[Kociolek_helper] BRAK KOCIOŁKA!!";
