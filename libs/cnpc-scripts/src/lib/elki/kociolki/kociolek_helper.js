...
function id(name) {
  if (typeof name == 'string')
    return (
      _GUI_IDS[name] || (_GUI_IDS[name] = Object.keys(_GUI_IDS).length + 1)
    );

  if (typeof name == 'number') {
    var keys = Object.keys(_GUI_IDS);
    for (var i = 0; i < keys.length; i++)
      if (_GUI_IDS[keys[i]] == name) return keys[i];
  }
}
...
function baseGui(withoutBackground) {
  try {
    ...
    var x = gui
      .addLabel(id('l-recipeInfo'), '§c[P]', 265, 0, 12, 12)
      .setHoverText([
        '§7Obok będzie wyświetlany przepis.',
        '§7Po 20 linijek na kartę',
      ]); // 56
    ...
    gui.setBackgroundTexture(
      withoutBackground ? TEXTURES.guiEmpty : TEXTURES.guiFancy
    );
    ...
    if (elki.side && elki.side.list.length > 0)
      gui
        .addButton(id('b-mergeCrucibles'), '§a[+]', 290, 40, 12, 12)
    ...
    if (sdata.has('przepis')) {
      ...
      while (przepis.length > 0) final.push(przepis.splice(0, 21));
...
_GUI = {
  ...
  continueBrewing: new Gui(
    ...
    function (e) {
      ...
      gui
        .addLabel(id('l-addItem'), '§4Wrzuć fiolkę', 85, 62, 55, 20)
        .setHoverText('§7Wrzuć fiolkę ze zlanym eliksirem'); // id 9
      gui.addItemSlot(100, 20);

      gui
        .addLabel(id('l-addRecipe'), '§4Dodaj Przepis', 85, 82, 55, 20)
        .setHoverText(
          '§7Dodaj item z przepisem, pojawi się po prawej stronie!'
        ); // id 10
      ...
      gui
        .addButton(id('b-removeRecipe'), '§c✖', 160, 83, 20, 20)
        .setHoverText(['§7Usuwa przepis z podglądu']); // id 111
  ...
  eliksirCreatorMenu: new Gui(
    'eliksirCreatorMenu',
    function (e, current, show) {
      // player, current, show
      var gui = baseGui();
  ...
  ingredientSelector: new Gui(
    'ingredientSelector',
    function (e, type, search) {
      var gui = baseGui(true);
      ...
      if (!type) {
        ...
        gui
          .addLabel(id('l-warning'), '§4§l[UWAGA]', 5, 5, 80, 30)
          .setHoverText([
        ...
        if (!skladniki || !skladniki.result || skladniki.error)
          return e.player.message(
            '[§cDebugger§f] §7Error, napisz do administracji! -' +
              skladniki.error
          );
  ...
  stepList: new Gui(
    ...
    function (e) {
      ...
      gui
        .addLabel(id('l-info1'), '§3[I]', 230, 20, 15, 15)
  ...
  modifyIngredient: new Gui(
    ...
    function (e, skladnik) {
      ...
      gui
        .addLabel(id('l-lm'), '§4[LM]', 10, 10, 20, 20)
      ...
      for (var i = 0; i < items.length; i++) {
        ...
        if (lore.length >= 2)
          if (lore[0].toLowerCase() == '§askładnik eliksiru')
            skladniki.push(
              items[i].getDisplayName() + ' §7[' + lore[1] + '§7]'
            );
      }
  ...
  eliksirInocreation: new Gui(
    ...
    function (e) {
      ...
      gui.addLabel(
        id('l-info'),
        '§9Wybierz eliksir do inokreacji',
        70,
        0,
        160,
        20
  ...
  labelMaker: new Gui(
    ...
    function (e, eliksir) {
      ...
      if (!eliksir) {
        ...
        gui
          .addLabel(id('l-info1'), '§c[I]', 5, 10, 100, 20)
          .setHoverText([
            '§7Aby dodać kolor:',
...
function setWait(elki, ile) {
  if (!elki || !ile || ile <= 0 || isNaN(ile)) return false;
...
function addElement(e, handleaddOrSkladnik, text) {
  if (handleaddOrSkladnik == false) {
    ...
    var _t = ACTIONS[text];
    if (x.ile) _t = _t.replace(/\_\_/g, x.ile);
...
function handleAdd(e, text) {
  ...
  if (akcja.indexOf('ogień') > -1) {
    ...
    if (
      (turnOn && elki.current.ignis == true) ||
      (!turnOn && elki.current.ignis == false)
    )
  ...
  if (!akcja) return e.player.message('[§cEliksiry§f] §7brak guzika!');
  ...
  if (label.indexOf('temperaturę') > -1) {
    ...
    var withAction = addToList(
      elki,
      akcja + ' [' + ileText + ' ' + labelText + ']',
      e.player
    );
...
function checkArray(array, text) {
  ...
  for (var x = 0; x < lastFive.length; x++) if (lastFive[x] == text) i++;
