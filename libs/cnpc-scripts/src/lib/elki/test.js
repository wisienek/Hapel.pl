if (loreArray.toString().contains("Consumable")) {
    ...
    var now = Date.now();
    if( cld && cld > now ){ return e.player.message("[§cAdmin§f] §7This item is still on cooldown: §c"+ parseInt((cld-now)/1000) +"§7 seconds" ); }
    ...
    if (playerHealth == playerMaxHealth) {
        ...
        executeCommand('tellraw ' + player + ' ["",{"text":"§cYou are already at full health."}]');
        ...
        executeCommand('tellraw ' + player + ' ["",{"text":"§bYou have been healed."}]', player);
