...
function confsprzedaj(e, sprzedaje) {
  ...
  if (!skladnik || skladnik.error || skladnik.result.length == 0) {
    return e.player.message(skladnik.error ? "[§cDebugger§f] §7Napotkałeś błąd: " + skladnik.error : "[§cZielarz§f] §7Nie znaleziono składnika na liście!")
  }
...
function kup(e, opcja) {
  ...
  if (opcja) {
    ...
    if (!skladniki || skladniki.error || skladniki.result.length == 0) {
      return e.player.message("[§cDebugger§f] §7Napotkałeś błąd: " + skladniki.error)
    }
    ...
    switch (e.buttonId) {
      ...
      case 4: {
        ...
        if (typeof playerWallet == "object") {
          e.player.message("[§cDebugger§f] §7Wystąpił błąd podczas pobierania portfela gracza: " + playerWallet.error);
      ...
      case 10: {
        ...
        if (!ile) {
          return e.player.message("[§cZielarz§f] §7Podałeś nieodpowiednią liczbę!")
        }
        if (ile <= 0 || ile > 5000) {
          return e.player.message("[§cZielarz§f] §7Liczba powinna miścić się w przedziale (0;5000> !")
        }
        var pdata = e.player.getTempdata();
        var koszyk = pdata.get("koszyk");
        koszyk = JSON.parse(koszyk) || {};
      ...
      case 21: {
        ...
        if (!item) {
          ...
          e.player.message("[§cZielarz§f] §7Nie znaleziono przedmiotu sprzedawanego w ekwipunku!");
          return menu(e);
        }
        ...
        if (!wart) {
          return e.player.message("[§cZielarz§f] §7Niepoprawna jednostka wartościowa!")
        }
  ...
  switch (e.scrollId) {
    case 1: {
      ...
      if (ilosc > 5000 || ilosc <= 0) {
        return e.player.message("[§cZielarz§f] §7Ilość musi być z przedziału (0;5000>!")
      }
...
function getItemName(kat, sel) {
  ...
  if (kat == "Zwierzęce") {
    ...
    if (sel.indexOf("igła") > -1 || sel.indexOf("dziób") > -1 || sel.indexOf("kieł") > -1 || sel.indexOf("kolec") > -1 || sel.indexOf("żądło") > -1) {
      item = "harvestcraft:shrimpcookeditem"
    }
    if (sel.indexOf("jad") > -1 || sel.indexOf("łza") > -1 || sel.indexOf("śluz") > -1) {
      item = "harvestcraft:blueberryjuiceitem"
    }
    if (sel.indexOf("jajko") > -1 || sel.indexOf("jajo") > -1 || sel.indexOf("jajeczko") > -1) {
      item = "minecraft:egg"
    }
    if (sel.indexOf("pancerz") > -1 || sel.indexOf("skorupa") > -1 || sel.indexOf("łuska") > -1) {
      item = "minecraft:shulker_shell"
    }
    if (sel.indexOf("włos") > -1 || sel.indexOf("futro") > -1 || sel.indexOf("skóra") > -1) {
    ...
    if (sel.indexOf("liść") > -1 || sel.indexOf("listki") > -1 || sel.indexOf("liście") > -1) {
      item = "harvestcraft:tealeafitem"
    }

    if (sel.indexOf("ropa") > -1 || sel.indexOf("sok") > -1 || sel.indexOf("oliwa") > -1 || sel.indexOf("olej") > -1 || sel.indexOf("wywar") > -1 || sel.indexOf("wyciąg") > -1) {
      item = "harvestcraft:papayajuiceitem"
    }
    if (sel.indexOf("nasiona") > -1 || sel.indexOf("pestki") > -1 || sel.indexOf("słonecznik") > -1 || sel.indexOf("ziarna") > -1) {
      item = "minecraft:wheat_seeds"
    }
    if (sel.indexOf("sproszkowan") > -1 || sel.indexOf("suszon") > -1 || sel.indexOf("cukier") > -1) {
